...
| B | D5-D9 | RRF 融合公式（D1）+ fused-rank budget cut（G1）+ E1 lexical priority + 10 streams emit | 既有 score 仍 emit；diagnostic shape 加 `fused_rank`；K1.1-off ≥ 70%（融合本身贡献）|
...
| **F** | Fusion + budget cut | **重塑** | "linear fusion + rerank stage" → "RRF over 10 streams + fused-rank budget cut" |
...
- **10 streams**（详见 `DECISION-01 § 2` + B.B4 diagnostic closure）：
...
| Era 1 ID | 描述 | β 处理 |
|---|---|---|
| F1 | Candidate fusion stage 显式化 | **重塑为 B.B1**（RRF over 10 streams 而非 "linear fusion"）|
