...
> - `.do-it/findings/v0.3.10/04-codex-cross-reference.md` — Codex 8 finding
...
- 加 reviewer_trust_mode awareness：trust_mode=automatic 时 used 权重 × 0.5
...
### Acceptance
- 单测：100 次相同 used 报告不让 RECALLS edge count 线性增长
- audit log 中 path expansion 每个 candidate 有 seed_id record
- 文档 prose 同步
...
### Target
- 改 `docs/v0.3/v0.3.9/README.md:63` 和 `decisions.md:21` 反映真实状态
...
### Target
- 用 `feedback_bench_default_full_set` 流程（setsid + disown）跑全集
...
### Target
- 跑 LoCoMo full embedding-on + embedding-off
...
| Cat-G2 claim_kind 扩展数 | Phase 3 决定 | 9 kind 全扩 / 5 kind 收紧 |
| Cat-G1 governance route 合并候选 | Phase 3 prose 文档化后评估 | should ≤ 3 routes 是否达 |
