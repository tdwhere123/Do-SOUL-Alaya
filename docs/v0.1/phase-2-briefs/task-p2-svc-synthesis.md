...
|---|---|---|
| `vendor/do-what-new-snapshot/packages/core/src/synthesis-service.ts` | `packages/core/src/synthesis-service.ts` | Copy first; apply only package-name/path rewrites and adapter points in §2.3. |
| `vendor/do-what-new-snapshot/packages/core/src/__tests__/synthesis-service.test.ts` | `packages/core/src/__tests__/synthesis-service.test.ts` | Copy first; apply only package-name/path rewrites and adapter points in §2.3. |
...
|---|---|---|
| `SynthesisSseBroadcaster` with `broadcastEntry(entry)` | `SynthesisRuntimeNotifier` with `notifyEntry(entry)` | Invariant §11 forbids SSE transport; Phase 2 keeps only in-process runtime notification semantics. |
