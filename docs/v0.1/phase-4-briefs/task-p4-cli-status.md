# Implementation Brief: Task P4-cli-status — Implement alaya status

> - **Phase**: 4
> - **Wave**: 4
> - **Card ID**: P4-cli-status
> - **Port mode**: requires-redesign
> - **Source**: `n/a`
> - **Target**: `apps/core-daemon/src/cli/status.ts`, `apps/core-daemon/src/__tests__/cli-status.test.ts`
> - **Size**: M
> - **Prerequisite**: P4-cli-bridge, P4-trust-state
> - **Blocks**: Gate-4 demo
...
`docs/v0.1/phase-4-briefs/README.md` row "P4-cli-status";
...
**Goal**: Deliver implement alaya status.
...
|---|---|---|
| `n/a` | `apps/core-daemon/src/cli/status.ts` | Alaya-specific redesign; tests must prove each behavior listed below. |
| `n/a` | `apps/core-daemon/src/__tests__/cli-status.test.ts` | Alaya-specific redesign; tests must prove each behavior listed below. |
...
| AC4 | Relevant targeted tests pass | `rtk pnpm exec vitest run --project @do-soul/alaya-core-daemon -t "cli status"` |
| AC5 | Completion report captures source files, port mode, verification, deviations, and deferrals | `docs/v0.1/phase-4-briefs/reports/task-p4-cli-status.md` exists and cites backlog issues for any deferred scope |
...
5. `rtk pnpm exec vitest run --project @do-soul/alaya-core-daemon -t "cli status"`
