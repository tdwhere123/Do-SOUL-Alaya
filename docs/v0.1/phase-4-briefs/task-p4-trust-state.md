...
|---|---|---|
| `n/a` | `apps/core-daemon/src/trust-state.ts` | Alaya-specific redesign; tests must prove each behavior listed below. |
| `n/a` | `apps/core-daemon/src/__tests__/trust-state.test.ts` | Alaya-specific redesign; tests must prove each behavior listed below. |
...
5. `rtk pnpm exec vitest run --project @do-soul/alaya-core-daemon -t "trust state"`
