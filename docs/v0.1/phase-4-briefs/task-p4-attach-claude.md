...
- Profile mutation is two-step in one command: (1) MCP server entry
  written to `~/.claude.json` `mcpServers.alaya` block; (2)
  `/alaya-inspect` slash alias registered in the user's Claude Code
  slash-command registry (the implementation MUST detect the active
  registry path that matches the installed Claude Code version and
  document it in the completion report) bound to the shell command
  `alaya inspect --open`.
