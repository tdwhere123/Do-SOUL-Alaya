...
| EventLog ↔ trust-state cross-await orphan window (data / architecture / test cross-vote) | New `GardenTaskKind.EVENT_LOG_ORPHAN_DETECTION` reuses the existing `orphan_radar` framework via migration 057. The Auditor scans `event_log` for trust delivery / usage events whose `audit_event_id` is missing from both trust tables and emits `soul.garden.event_log_orphan_detected` for Inspector observability. `publishWithMutation` API and its 11 callsites are untouched. |
| Trust counter persistence drift (`#BL-020`) | `rebuildCountersFromEventLog` in `packages/core/src/trust-state-service.ts` reconstructs installed / configured / unverifiable counters from EventLog at daemon startup before `markReady`. `trust-state-persistence.test.ts` asserts counter rebuild across daemon restart. `#BL-020` is closeable. |
...
| `OPENAI_API_KEY` secret-ref naming hazard | `.env` field renamed to `ALAYA_OPENAI_SECRET_REF`. Daemon startup reads the new field with `OPENAI_API_KEY` fallback for backward compatibility, resolves `env:` / `file:` references through `readOptionalSecretEnv`, then writes the resolved plaintext back to `process.env.OPENAI_API_KEY` so downstream provider integrations that read the standard env name keep working without further changes. |
| Cross-document readiness drift (Gate-4 closeout vs INDEX vs report) | `INDEX.md` line 30 + line 116 flip P4-trust-state to `live-event-ready`. `gate-4-closeout.md:10` narrows the `#BL-015` closure note to "delivery/usage records" — counter persistence still tracked by `#BL-020` (now resolvable through EventLog replay). `task-p4-trust-state.md §2.1` table aligned with code reality. |
...
- `docs/v0.1/phase-1-followup-briefs/task-p1-protocol-followup-grace-and-trust-counters.md`
  ...
  counter events (phase-4a) added under `#BL-013` / `#BL-020`
  preparation, with the events kept Alaya-original (source `n/a`).
