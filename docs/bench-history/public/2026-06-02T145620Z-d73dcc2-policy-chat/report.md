...
- Run at: 2026-06-02T14:56:20.327Z
...
Release hard gates:
- ✗ longmemeval_s_500_embedding_off_r_at_5 LongMemEval-S 500 embedding-off R@5: 81.60% < target 90.00%
- ✓ longmemeval_s_non_monotonic_rate non_monotonic_rate: 0.00% <= target 10.00%
- ✗ longmemeval_s_budget_dropped_max_entries budget_dropped_entries: 143 > target 8
- ✓ longmemeval_s_candidate_absent candidate_absent: 6 <= target 6
- ✓ longmemeval_s_evidence_stream_gold_delivery evidence stream gold delivery: 100.00% >= target 15.00%
- ✗ recall_p95_embedding_off recall p95 embedding-off: 4807.884669ms > target 200ms
...
- R@1: 52.00% (95% CI ±4.36pp, [47.62%, 56.35%])
- R@5: 81.60% (95% CI ±3.39pp, [77.97%, 84.75%])
- R@10: 83.60% (95% CI ±3.24pp, [80.10%, 86.59%])
- Latency p50: 1902.050227 ms
- Latency p95: 4807.884669 ms
- Token saved vs full-prompt baseline: 100.00%
- Per-recall token economy (500 calls, measure-only):
  - delivered_context_tokens: mean=487.3 p50=483.0 p95=548.0 max=708
  - coarse_pool_size: mean=768.3 p50=760.5 p95=977.1 max=1100
  - fine_evaluated: mean=768.3 p50=760.5 p95=977.1 max=1100
  - fusion_streams_with_hits: mean=12.4 p50=12.0 p95=13.0 max=13
...
- Seed truncation: turns=6 answer_bearing=0 chars_clipped=62602
- Seed extraction path: official_api_compile (cache_hits=124366 llm_calls=0 offline_fallbacks=0 live_failures=0 cached_failures=0 facts=914234 signals_dropped=4332 [parse_dropped=2368 compile_overflow_dropped=1])
  - ⚠ 4332 extracted signal(s) were lost before seeding (2368 dropped by the parser as malformed / over the 64-signal cap, 1 dropped by compile() as oversized, 1963 dropped when a seed-materialization batch failed); a dropped answer-bearing signal inflates the miss rate.
- Quality metrics: non_monotonic=0.00% (0/500) budget_drop_loss=20 budget_dropped_entries=143 candidate_absent=6 no_gold=0 evidence_gold=100.00% path_top10=45.74%
- Abstention (calibrated confidence, threshold=0.91): 30 questions, correct@1=23 correct@5=21 correct@10=21; these correct-at-k counts are credited to the recall@k numerator (denominator unchanged).
