...
Release hard gates:
- ✓ longmemeval_s_500_embedding_on_r_at_5 LongMemEval-S 500 embedding-on R@5: 85.20% >= target 55.00%
...
- ✗ longmemeval_s_budget_dropped_max_entries budget_dropped_entries: 140 > target 8
- ✓ longmemeval_s_candidate_absent candidate_absent: 1 <= target 6
- ✓ longmemeval_s_evidence_stream_gold_delivery evidence stream gold delivery: 100.00% >= target 15.00%
- ✓ recall_p95_embedding_on recall p95 embedding-on: 542.94619ms <= target 1100ms
...
- R@1: 59.20% (95% CI ±4.29pp, [54.84%, 63.42%])
- R@5: 85.20% (95% CI ±3.11pp, [81.82%, 88.04%])
- R@10: 89.80% (95% CI ±2.66pp, [86.84%, 92.16%])
- Env embedding R@5 overall: 85.20%
- Env embedding R@5 when provider returned: 85.20%
- Latency p50: 364.897687 ms
- Latency p95: 542.94619 ms
- Token saved vs full-prompt baseline: 100.00%
- Per-recall token economy (500 calls, measure-only):
  - delivered_context_tokens: mean=405.4 p50=364.5 p95=736.0 max=1007
  - coarse_pool_size: mean=380.6 p50=364.0 p95=564.1 max=727
  - fine_evaluated: mean=380.6 p50=364.0 p95=564.1 max=727
  - fusion_streams_with_hits: mean=13.6 p50=13.0 p95=15.0 max=15
...
- Seed extraction path: official_api_compile (cache_hits=124366 llm_calls=0 offline_fallbacks=0 live_failures=0 cached_failures=0 facts=296279 signals_dropped=36450 [parse_dropped=418 compile_overflow_dropped=0 candidate_absent=34888 materialization_error=1144])
  - 34888 extracted signal(s) declined by governance routing (routed to evidence_only/deferred, not durable memory) — expected for conversational turns, not a failure.
  - ⚠ 1562 extracted signal(s) were lost before seeding (418 dropped by the parser as malformed / over the 64-signal cap, 0 dropped by compile() as oversized, 1144 failed materialization, 0 dropped when a seed-materialization batch failed); a dropped answer-bearing signal inflates the miss rate.
- Quality metrics: non_monotonic=0.00% (0/500) budget_drop_loss=10 budget_dropped_entries=140 candidate_absent=1 no_gold=7 evidence_gold=100.00% path_top10=93.14%
- Abstention (calibrated confidence, threshold=0.91): 30 questions, correct@1=27 correct@5=26 correct@10=26; these correct-at-k counts are credited to the recall@k numerator (denominator unchanged).
