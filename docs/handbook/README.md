...
当前仓库处于 reset / extraction 后的 ALA-R1-R9 runtime/activation/operations contracts 状态：旧原型实现已被有意删除，`docs/archive/`
仅保留历史材料，不代表现行实现；当前实现覆盖 root package、runtime/API、audit、storage baseline、doctor、
Memory Ontology/Evidence、Structure Registry/Paths、Governance/Promotion、Recall/Context、Provider/Proposal、Session Audit/Trust、Integration/MCP descriptors、CLI fallback、Attach/Profile preview/confirm、Gateway envelope、Operations/Portability。
...
| Runtime readiness | `runtime-status.md` | ALA-R1-R9 runtime/activation/operations readiness 与 live daemon/MCP transport/profile write/Gateway runner/Inspector/benchmark 缺口边界。 |
