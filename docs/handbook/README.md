...
当前仓库处于 reset / extraction 后的 ALA-R1 baseline 状态：旧原型实现已被有意删除，`docs/archive/`
仅保留历史材料，不代表现行实现；当前实现只覆盖 root package、runtime/API、audit、storage baseline 与 doctor。
...
| Runtime readiness | `runtime-status.md` | ALA-R1 baseline readiness 与未实现 surface 的边界。 |
