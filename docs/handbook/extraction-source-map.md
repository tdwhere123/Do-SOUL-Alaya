...
- 工程命名空间目标为 `@do-soul/alaya`。ALA-R1 已引入 root package、
  ...
  recall/context、provider/proposal、session trust runtime contracts；ALA-R8/R9 已引入
  integration/MCP descriptors、CLI fallback normalization、Attach/Profile preview/confirm、
  Gateway envelope、Operations/Portability contracts。当前仍没有 live daemon、live MCP
  transport/server、真实 profile 文件写入 runner、Gateway runner、真实外部 provider adapter、
