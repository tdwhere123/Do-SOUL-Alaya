...
| Garden engine | `packages/soul/src/garden/`, `packages/soul/src/shared/bootstrapping-ids.ts` | partial: `bootstrapping-service.ts`, `session-override-remediation.ts`, `backlog-telemetry.ts`, and `shared/bootstrapping-ids.ts` are ported; remaining P2 Garden batches are not yet ported |
