...
| Phase 1 | Wave 1 leaves: protocol, migrations, storage shared, config, topology, CLI shell, engine-gateway | not-started | Gate-1 |
| Phase 2 | Wave 2: storage repos batches + core services + Garden + security defense | not-started | Gate-2 |
| Phase 3 | Wave 3: ConversationService, MCP tooling, MCP discovery, run lifecycle, misc services | not-started | Gate-3 |
...
| MCP tool surface | `not-started` | `mcp-consumable` | P3-mcp-tooling + P4-mcp-server |
| Core daemon | `not-started` | `live-event-ready` | P4-daemon-core |
...
| Operations (backup, portable, status) | `not-started` | `live-event-ready` | (will land alongside P4-daemon-core) |
...
- **Gate-1**: All Phase 1 leaves ported and `pnpm build` + `pnpm test`
  pass.
...
- **Gate-4**: `alaya install` → `alaya attach codex` → MCP tool call
