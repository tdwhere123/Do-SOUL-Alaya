...
daemon, MCP, CLI, or Inspector route. v0.3.3 adds CLI-consumable
doctor health for memory graph edges and path relation counts, but does
not promote the graph inspector contract to a live route. Daemon default
bootstrap templates are empty; explicit templates can still be planted,
and empty defaults reconcile as `skipped_no_templates`. P5-e2e is
