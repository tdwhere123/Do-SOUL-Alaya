...
| Durable recall fan-in (Route 乙, R2) | `implementation_wired`. The temporary `session_cohort_fanin` heuristic is retired; durable ACCEPTED positive `memory_entry <-> memory_entry` co-occurrence hub edges (member -> representative) carry fan-in. Direct hub effects score through `path_expansion`; `graph_expansion` only covers leftover/multi-hop reach. The structural reserve is gold-blind relevance-gated and honors suppression; the representative-selection guard nominates ONE query/evidence-relevant representative and refuses membership-only promotion. **R@5 -> 90% efficacy is UNPROVEN locally and deferred to the R5 500q gate.** | `packages/core/src/recall-service.ts`; `packages/core/src/recall/graph-expansion.ts`; `packages/core/src/recall/fusion-delivery.ts`; `packages/core/src/recall-service-types.ts`. |
