...
| Repository mode | Foundation contracts | 旧实现已删除；当前以独立 ALA-R1-R4 package/runtime foundation + handbook + v0.1 planning + archive 为主 |
...
| Runtime/API | `foundation-contracts-ready` | `src/index.ts` exports `createAlayaRuntime(...)` and `AlayaRuntimePort`; public state changes cover audited R1 decisions plus R2 ontology writes, R3 path/manifestation/topology operations, and R4 governance decisions while callback-based audited mutation orchestration stays internal |
| Storage/migration | `foundation-contracts-ready` | `src/storage/sqlite.ts` is internal, initializes `alaya.sqlite`, `alaya_migrations`, audit tables, and ordered `002-ontology`, `003-structure`, `004-governance` migrations |
| Doctor CLI / status | `foundation-contracts-ready` | `dist/cli/index.js doctor --data-dir ...` emits JSON with package/runtime/storage/ontology/structure/governance `ok`, profile/provider `not_implemented`, and `product_ready: false` |
...
| Build/Test/Run readiness | `foundation-contracts-ready` | R1-R4 gate verified with `rtk pnpm build`, `rtk pnpm test`, doctor smoke, import scan, and `rtk git diff --check` |
...
- 不把 ALA-R1 doctor 的 `r1_baseline_ready: true` 或 ALA-R2/R3/R4 doctor 的 `foundation_contracts_ready: true` 写成完整产品 ready、agent usage proof 或 provider/recall ready。

## Current Verification Boundary

ALA-R1 以后，build/test gates 适用于当前 root package/runtime foundation。当前可执行的 R1-R4 gate 是：
...
R1-R4 gate 只能证明 package/runtime/storage/audit/doctor plus ontology/structure/governance foundation contracts。MCP、Attach/Profile、Gateway、recall/provider、usage proof、Inspector、benchmark 和 full product loop 仍需后续 cards。

## Transition Gate (When Implementation Returns)

只有在以下条件同时满足后，状态才可从 `foundation-contracts-ready` 上调到更高产品 readiness：
