...
| Write-side Keys / facets | Daemon materialization router | Facet tags default OFF (`ALAYA_RECALL_FACET_TAGS`). Projection routing default OFF (`ALAYA_RECALL_PROJECTION_ROUTING`). Read-side `ALAYA_RECALL_PROJECTIONS` is a different flag (scoring floors). Missing facet supply seals `inactive:supply_unavailable`. |
...
| `ALAYA_RECALL_PROJECTION_ROUTING` | **Write-side** lift of a projection-bearing `signal_only` signal to `memory_entry_only`. Default **OFF** unless `1\|true\|on\|yes` (`apps/core-daemon/src/runtime/recall-materialization/recall-materialization-router.ts`). Product opt-in; not a hidden bench switch. |
| `ALAYA_RECALL_FACET_TAGS` | **Write-side** content-derived `facet_tags` for the `facet_overlap` stream. Default **OFF**. Absence is sealed `inactive:supply_unavailable` when the query seeks facets and the packet has no persisted tags. |
