...
| Write-side Keys / facets | Daemon materialization router | Closed-vocab facet-tag write and the `facet_overlap` scoring stream are deleted (0/25355 tags; remaining misses need different Keys). Write-side `signal_only` lift for `scope` / `task_scope` / `workflow_preference` is deleted (no producer emits those kinds with projection payloads). Read-side `ALAYA_RECALL_PROJECTIONS` remains scoring floors, default ON. Query-side `FACET_VOCABULARY` still feeds slice keys and demand atoms. |
