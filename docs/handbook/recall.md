...
| Write-side Keys / facets | Daemon materialization router | Same env name, opposite defaults — see below. |
...
| Name | Live fact | Cite |
| --- | --- | --- |
| `ALAYA_RECALL_PROJECTIONS` | **Split semantics.** Write-side daemon defaults **OFF** unless `1\|true\|on\|yes` (`apps/core-daemon/src/runtime/recall-materialization/recall-materialization-router.ts:125-129`). Read-side core defaults **ON** unless `0\|false\|off\|no\|disabled` (`packages/core/src/config/recall-runtime-config.ts:16-18,53`). Read-side ON is intent/preference scoring, not write-side facet supply. B dump: `facet_overlap` ranks 0. |
