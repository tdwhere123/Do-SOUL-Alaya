...
| Runtime status | ALA-R1-R9 runtime/activation/operations readiness、live daemon/MCP transport/profile write/Gateway runner/Inspector/benchmark 明确保留未实现、build/test gate 只绑定当前 root package/runtime contracts | 声称 live daemon、live MCP transport、真实 profile 文件写入、Gateway runner、真实外部 provider adapter、Inspector、benchmark 或 full product 已通过或可执行 |
