...
本页定义 Do-SOUL Alaya 在当前 reset/extraction + ALA-R1-R9 runtime/activation/operations contracts 阶段的执行纪律。
...
- 当前实现文件缺失的 surface（live daemon、live MCP transport、真实 profile 文件写入、Gateway runner、真实外部 provider adapter、Inspector、benchmark 等）不要发明 build/test/CLI/MCP/smoke 命令；只能记录对应 surface 恢复后的 gate 条件。
...
ALA-R1-R9 package/runtime/activation/operations contracts 已重新引入，root package 的 build/test/doctor gate 适用。若任务要求未实现 surface 的 build/test/smoke（例如 live daemon、live MCP transport、真实 profile 文件写入、Gateway runner、真实外部 provider adapter、Inspector、benchmark），报告 `NOT_VERIFIED` 或 `BLOCKED`，不要假造命令。
