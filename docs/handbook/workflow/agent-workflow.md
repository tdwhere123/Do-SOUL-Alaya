...
- 当前实现文件缺失时，不要发明 build/test/CLI/MCP/smoke 命令；只能记录 package surface 恢复后的 gate 条件。
- 不恢复旧 prototype source，不导入 `@do-what/*` 或 `do-what-new/packages/*` runtime code。
- 发现范围冲突、事实冲突、或写入受限时，立即返回 `BLOCKED`。
- 不用大范围重写；优先修正最小一致面。

## Verification In Docs Reset State

当前 docs reset 阶段的有效验证包括：
...
Build/test gates 只在 package surface 重新引入后适用。若任务要求 build/test 但当前没有实现面，报告 `NOT_VERIFIED` 或 `BLOCKED`，不要假造命令。
