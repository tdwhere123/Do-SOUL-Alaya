...
> - **Target**: `apps/core-daemon/src/__tests__/phase6-agent-use-protocol.test.ts`, `apps/core-daemon/src/__tests__/e2e/v0.1-release-loop.test.ts`, `apps/core-daemon/src/mcp-memory-tool-catalog.ts`, `apps/core-daemon/src/profile-mutation.ts`, `README.md`, `README.zh-CN.md`
> - **Size**: M
> - **Prerequisite**: none
...
|---|---|---|
| AC1 | MCP/CLI loop sequence is explicit and ordered (recall -> usage receipt -> proposal -> review). | `apps/core-daemon/src/__tests__/phase6-agent-use-protocol.test.ts` transcript assertions and step order checks. |
| AC2 | CLI fallback tool discovery matches MCP tool discovery. | `apps/core-daemon/src/__tests__/phase6-agent-use-protocol.test.ts` (`alaya tools list --json` equals MCP `tools/list`). |
| AC3 | Pending-proposal visibility is consistent before and after review. | `apps/core-daemon/src/__tests__/phase6-agent-use-protocol.test.ts` assertions over `soul.list_pending_proposals` and `alaya review pending --json`. |
...
```bash
rtk rg -n "Phase-6 MCP agent-use protocol proof|soul.report_context_usage|soul.list_pending_proposals|soul.review_memory_proposal" apps/core-daemon/src/__tests__/phase6-agent-use-protocol.test.ts
...
- Shared with `P6-live-agent-proof`: `apps/core-daemon/src/__tests__/phase6-agent-use-protocol.test.ts`.
