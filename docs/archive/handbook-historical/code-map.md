...
| Memory Inspector backend | `apps/inspector/src/`, `apps/inspector/web/src/` | P4-inspector-server is `live-event-ready`: loopback Hono server, token middleware, config/graph/status routes, daemon-proxied embedding supplement reads/writes, paste-to-file secret refs, sanitized route errors, and static bundle host. Config writes are audited through the daemon EventLog path. v0.3.10 adds the manifestation budget config proxy/UI and the `/api/bench-trend` backend plus `/bench-trend` page for 30-day bench history panels. Source-restructure keeps inspector-web runtime files grouped under `app/`, `api/`, `components/`, `i18n/`, `pages/`, `types/`, and `utils/`, moves page-local graph rendering helpers under `apps/inspector/web/src/pages/graph-page/`, and groups frontend tests under the aggregated `apps/inspector/web/src/__tests__/` tree instead of mixing them into runtime directories. |
