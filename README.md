...
- 旧本地原型实现已按计划删除。
- ALA-R1 已重新引入独立的 `@do-soul/alaya` package/runtime truth kernel。
- ALA-R2/R3/R4 已落地 foundation contracts：Memory Ontology/Evidence、
  Structure Registry/Paths、Governance/Promotion，均通过 runtime-owned audited
  operations 与 internal SQLite migrations 进入当前包边界。
- ALA-R5/R6/R7 已落地 runtime use proof contracts：Recall/Context Assembly、
  Provider/Proposal、Session Audit/Trust，均保持 runtime artifact 与 durable
  truth 分离。
- ALA-R8/R9 已落地 activation/operations contracts：Integration operation
  descriptors、MCP surface descriptors、CLI fallback normalization、Attach/Profile
  preview/confirm、Gateway envelope、profile/secret/provider status、portable
  bundle、backup metadata 与 read-only operations status。
...
当前实现覆盖 ALA-R1 到 ALA-R9 的 runtime/package/activation/operations contracts；
daemon、live MCP transport、真实 profile 文件写入、真实外部 provider adapter、
Inspector、benchmark 与完整产品闭环仍未实现。
