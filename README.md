...
- ALA-R1 已重新引入独立的 `@do-soul/alaya` package/runtime truth kernel
  baseline：public runtime API、audit-first mutation helper、SQLite migration
  baseline、doctor CLI 与 focused tests。
...
当前只实现 ALA-R1 runtime truth kernel baseline；daemon、MCP、
