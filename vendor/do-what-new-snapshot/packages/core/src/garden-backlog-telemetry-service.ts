...
export interface GardenBacklogTelemetrySseBroadcasterPort {
...
export interface GardenBacklogTelemetryServiceDependencies {
  ...
  readonly sseBroadcaster?: GardenBacklogTelemetrySseBroadcasterPort;
...
export class GardenBacklogTelemetryService {
  ...
  private async publishSnapshotSafely(generation: number): Promise<void> {
    ...
    void this.broadcastSnapshotBestEffort(entry);
  }
  ...
  private async broadcastEntry(entry: EventLogEntry): Promise<void> {
  ...
  private async broadcastSnapshotBestEffort(entry: EventLogEntry): Promise<void> {
    try {
      await this.broadcastEntry(entry);
    } catch (error) {
      this.warn("garden backlog snapshot broadcast failed", {
        error: toErrorMessage(error)
      });
    }
  }
  ...
  private async finishWarningSideEffectsBestEffort(args: {
    ...
    try {
      await this.broadcastEntry(args.entry);
    } catch (error) {
      this.warn("garden backlog warning broadcast failed", {
