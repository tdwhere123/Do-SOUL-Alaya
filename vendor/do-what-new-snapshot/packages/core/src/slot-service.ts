...
export interface SlotSseBroadcaster {
...
export interface SlotServiceDependencies {
  ...
  readonly sseBroadcaster: SlotSseBroadcaster;
...
export class SlotService {
  ...
  public async onClaimActivated(
    ...
    if (sameScopeSlot !== null) {
      return await this.evaluateSameScopeElection(sameScopeSlot, parsedClaim, deferredBroadcastEvents);
    }

    const relatedSlots = await this.findRelatedSlots(parsedClaim);

    if (relatedSlots.length === 0) {
      const created = await this.createSlotForClaim(parsedClaim, deferredBroadcastEvents);
      return {
        slot: created,
    ...
    return await this.evaluateCrossScopeElection(relatedSlots, parsedClaim, deferredBroadcastEvents);
  }
  ...
  private async evaluateSameScopeElection(
    ...
    if (existingSlot.winner_claim_id === null) {
      const updated = await this.changeWinner(existingSlot, claim.object_id, "seed_winner", TransitionCausedBy.SYSTEM, deferredBroadcastEvents);
      return {
        slot: updated,
        decision: "auto_won",
        reason: "seed_winner"
  ...
  private async evaluateCrossScopeElection(
    ...
    if (incumbent === null) {
      const created = await this.createSlotForClaim(claim, deferredBroadcastEvents);
      return {
        slot: created,
        decision: "new_slot_created",
        reason: "first_claim_for_subject"
  ...
  private async createSlotForClaim(claim: Readonly<ClaimForm>, deferredBroadcastEvents?: EventLogEntry[]): Promise<Readonly<Slot>> {
    const now = this.now();
