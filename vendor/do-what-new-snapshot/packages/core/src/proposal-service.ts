...
export interface ProposalServiceSynthesisServicePort {
  ...
  resolvePromotionDecision(
    ...
    options?: {
      ...
      // When provided, SSE broadcast is deferred: the event is pushed here
      // instead of being broadcast immediately. Callers must broadcast in order.
      readonly deferredBroadcastEvents?: EventLogEntry[];
    }
  ): Promise<Readonly<SynthesisCapsule>>;
}

export interface ProposalSseBroadcaster {
...
export interface ProposalServiceDependencies {
  ...
  readonly sseBroadcaster: ProposalSseBroadcaster;
...
export class ProposalService {
  ...
  public async createFromSynthesisPromotion(
    ...
    // broadcastEntry handles null run_id correctly (workspace-only broadcast).
  ...
  public async review(
    ...
    // Collect synthesis/claim SSE events during sub-service calls so that they
    // can be broadcast after all EventLog appends complete, preserving ordering.
    const deferredBroadcastEvents: EventLogEntry[] = [];

    if (parsedAction.action === "accepted") {
      await this.applyAcceptedReview(context, parsedAction.reviewed_at, deferredBroadcastEvents);
    } else {
      await this.applyRejectedReview(context, parsedAction.reviewed_at, deferredBroadcastEvents);
    }
    ...
    // Broadcast all deferred events in EventLog insertion order:
    //   reviewCreated → [synthesis/claim/slot sub-events] → reviewCompleted → resolved
    // broadcastEntry handles null run_id correctly (workspace-only broadcast), so claim
    // lifecycle events with run_id: null reach workspace SSE subscribers as intended.
    await this.broadcastDeferredEvents([reviewCreated, ...deferredBroadcastEvents, reviewCompleted, resolved]);
  ...
  private async applyRejectedReview(
    ...
    await this.dependencies.synthesisService.resolvePromotionDecision(
      ...
      "proposal_rejected",
      TransitionCausedBy.REVIEW,
      { cooldownUntil: addHours(reviewedAt, 24), deferredBroadcastEvents }
    );
  ...
  private async broadcastDeferredEvents(events: readonly EventLogEntry[]): Promise<void> {
