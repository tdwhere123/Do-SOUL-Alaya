...
export interface SignalSseBroadcaster {
...
export interface SignalServiceDependencies {
  ...
  readonly sseBroadcaster: SignalSseBroadcaster;
...
export class SignalService {
  ...
  public async receiveSignal(signal: CandidateMemorySignal): Promise<SignalServiceReceiveResult> {
    ...
    if (emittedEvent.run_id !== null) {
      await this.dependencies.sseBroadcaster.broadcastEntry(emittedEvent);
    }
    ...
    if (triagedEvent.run_id !== null) {
      await this.dependencies.sseBroadcaster.broadcastEntry(triagedEvent);
    }
    ...
    if (materialization.success !== true) {
      // No DB update in the failure path — SSE follows EventLog directly.
      if (matEvent.run_id !== null) {
        await this.dependencies.sseBroadcaster.broadcastEntry(matEvent);
      }
    ...
    if (materialization.target_kind === "deferred") {
      // EventLog-first: DB update then SSE (invariant #7).
      ...
      // Emit a corrective triage event so EventLog/SSE consumers see the final state.
      // The initial triaged event was broadcast with triage_result "accepted"; this
      ...
      if (deferredEvent.run_id !== null) {
        await this.dependencies.sseBroadcaster.broadcastEntry(deferredEvent);
      }
