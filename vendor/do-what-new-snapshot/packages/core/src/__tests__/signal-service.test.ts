...
import type { CandidateMemorySignal, EventLogEntry } from "@do-what/protocol";
...
describe("SignalService", () => {
  it("writes emitted/triaged events in order and moves accepted signals to triaged", async () => {
    ...
    const service = new SignalService({
      ...
      sseBroadcaster: {
        broadcastEntry: vi.fn(async (entry) => {
          order.push(`sse:${entry.event_type}`);
        })
      }
    });
    ...
    expect(order).toEqual([
      ...
      "repo:create",
      "sse:soul.signal.emitted",
      "event:soul.signal.triaged",
      "repo:update:triaged",
      "sse:soul.signal.triaged"
    ]);
  ...
  it("emits a corrective soul.signal.triaged deferred event after materialization router returns deferred", async () => {
    ...
    // the repo but no follow-up event was broadcast. SSE consumers were permanently
    // stuck on the initial "accepted" triage event. This verifies the corrective
    // soul.signal.triaged event with triage_result "deferred" is now appended and broadcast.
    const appendedEvents: Array<{ event_type: string; caused_by: string; payload_json: Record<string, unknown> }> = [];
    const broadcastedEventTypes: string[] = [];
    ...
    const service = new SignalService({
      ...
      sseBroadcaster: {
        broadcastEntry: vi.fn(async (entry) => {
          broadcastedEventTypes.push(entry.event_type);
        })
      },
      postTriageMaterializer: {
        materialize: vi.fn(async (signal) => ({
    ...
    // The corrective event must also be broadcast via SSE.
    const triagedBroadcasts = broadcastedEventTypes.filter((t) => t === "soul.signal.triaged");
    expect(triagedBroadcasts).toHaveLength(2); // initial "accepted" + corrective "deferred"
  });
});
