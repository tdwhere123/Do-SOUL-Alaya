...
import { HealthEventKind, Phase4AEventType, type EventLogEntry, type HealthJournalEntry } from "@do-what/protocol";
...
describe("HealthJournalService", () => {
  ...
  it("writes the expected event and repo payload", async () => {
    ...
    expect(sseBroadcaster.broadcastEntry).toHaveBeenCalledTimes(1);
  });
