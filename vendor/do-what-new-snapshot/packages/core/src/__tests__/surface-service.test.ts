...
function createDependencies(seed?: {
  ...
  return { dependencies, order, events, broadcastSpy, cascaderSpy, warnSpy, driftService };
}

describe("SurfaceService", () => {
  it("writes EventLog before creating surface", async () => {
    const { dependencies, order, events, broadcastSpy } = createDependencies();
  ...
  it("transitions status with EventLog-first order", async () => {
    const { dependencies, order, events, broadcastSpy, cascaderSpy } = createDependencies({
  ...
  it("cascades binding detach when status transitions to revoked", async () => {
    const { dependencies, cascaderSpy, broadcastSpy, driftService } = createDependencies({
      surfaces: [createSurface({ object_id: SURFACE_OBJECT_ID, surface_status: SurfaceStatus.ACTIVE })]
    });
    const service = new SurfaceService(dependencies);
  ...
  it("still fails closed on broadcast failure even when drift lease release also fails after the durable write", async () => {
    const { dependencies, driftService, warnSpy, broadcastSpy } = createDependencies({
      surfaces: [createSurface({ object_id: SURFACE_OBJECT_ID, surface_status: SurfaceStatus.ACTIVE })]
    });
    broadcastSpy.mockRejectedValueOnce(new Error("broadcast failed"));
    driftService.releaseLease.mockRejectedValueOnce(new Error("release failed"));
    const service = new SurfaceService(dependencies);
