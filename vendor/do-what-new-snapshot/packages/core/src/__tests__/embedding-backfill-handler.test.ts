...
import { MemoryDimension, ScopeClass, type MemoryEntry } from "@do-what/protocol";
import { EmbeddingBackfillHandler } from "../embedding-backfill-handler.js";
import type { MemoryEmbeddingRecord } from "@do-what/storage";
...
describe("EmbeddingBackfillHandler", () => {
  it("upserts only missing or changed hot-memory embeddings and skips unchanged content hashes", async () => {
    ...
    const upsert = vi.fn(async (record: MemoryEmbeddingRecord) => record);
  ...
  it("returns a deterministic skip when the embedding provider is unavailable", async () => {
    const handler = new EmbeddingBackfillHandler({
      ...
      memoryEmbeddingRepo: {
        findByObjectId: vi.fn(async () => null),
        upsert: vi.fn(async (record: MemoryEmbeddingRecord) => record),
        upsertIfContentHashMatchesCurrentMemory: vi.fn(async (record: MemoryEmbeddingRecord) => record)
      },
...
function createEmbeddingRecord(overrides: Partial<MemoryEmbeddingRecord>): MemoryEmbeddingRecord {
  return {
