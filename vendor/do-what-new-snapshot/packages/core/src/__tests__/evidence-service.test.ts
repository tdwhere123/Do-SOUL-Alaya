...
describe("EvidenceService", () => {
  it("writes soul.evidence.created before persistence and SSE broadcast", async () => {
    const order: string[] = [];
    ...
    expect(order).toEqual(["event_log", "repo_create", "sse"]);
  ...
  it("writes soul.evidence.health_changed and updates repo health", async () => {
    ...
    expect(order).toEqual(["event_log", "repo_update", "sse"]);
