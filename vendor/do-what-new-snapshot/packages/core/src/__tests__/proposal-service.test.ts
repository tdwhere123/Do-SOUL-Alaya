...
describe("ProposalService", () => {
  ...
  it("accept review transitions claim+synthesis and records karma gain", async () => {
    ...
    expect(claimTransitionSpy).toHaveBeenCalledWith(
      ...
      "proposal_accepted",
      "review",
      expect.objectContaining({ deferredBroadcastEvents: expect.any(Array) })
    );
  ...
  it("reject review transitions synthesis to rejected with cooldown and records penalty", async () => {
    ...
    expect(synthesisResolveSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        deferredBroadcastEvents: expect.any(Array)
      })
    );
    expect(proposalUpdateSpy).toHaveBeenCalledWith(
      "24c607da-7544-47a7-a28e-d649071f77f5",
  ...
  it("passes deferredBroadcastEvents to claimService so claim transition does not broadcast before reviewCreated", async () => {
    // Regression for P2-a: ClaimService.transitionLifecycle() must receive the
    // deferredBroadcastEvents array so it defers its broadcast rather than firing
    // before reviewCreated is broadcast. Verify the option is threaded through.
    ...
    const { dependencies } = createDependencies({
      claimService: {
        ...
        transitionLifecycle: vi.fn(async (_id, _state, _reason, _causedBy, options) => {
          ...
          if (options?.deferredBroadcastEvents) {
            options.deferredBroadcastEvents.push(deferredEventFromClaim);
          }
      ...
      sseBroadcaster: {
        broadcastEntry: vi.fn(async (entry: EventLogEntry) => {
          broadcastOrder.push(entry.event_type);
        })
      }
    });
    ...
    // reviewCreated must come before the claim lifecycle event in broadcast order.
    const reviewCreatedIdx = broadcastOrder.indexOf("soul.review.created");
    const claimTransitionIdx = broadcastOrder.indexOf("soul.claim.lifecycle_changed");
