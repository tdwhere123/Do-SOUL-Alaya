...
describe("BudgetBankruptcyService", () => {
  it("declares a soft bankruptcy, persists the proposal, updates current mode, and broadcasts both phase-3c events", async () => {
    ...
    expect(dependencies.sseBroadcaster.broadcastEntry.mock.calls.map(([entry]) => entry.event_type)).toEqual([
  ...
  it("keeps hard bankruptcies pending when no auto-applicable option exists", async () => {
    ...
    expect(
      dependencies.sseBroadcaster.broadcastEntry.mock.calls.filter(
...
function createDependencies(overrides: Partial<{
  ...
  sseBroadcaster: { broadcastEntry: BroadcastEntryMock };
}> = {}) {
