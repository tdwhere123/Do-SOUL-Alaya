...
function createDependencies(overrides: Partial<MemoryServiceDependencies> = {}): {
  ...
  readonly broadcastSpy: TestMock;
...
describe("MemoryService", () => {
  it("writes soul.memory.created before persistence and SSE broadcast with computed revision", async () => {
    const order: string[] = [];
    ...
    expect(order).toEqual(["event_query", "event_log", "repo_create", "sse"]);
  ...
  it("writes soul.memory.updated before persistence and SSE broadcast with computed revision", async () => {
    const order: string[] = [];
    ...
    expect(order).toEqual(["event_query", "event_log", "repo_update", "sse"]);
