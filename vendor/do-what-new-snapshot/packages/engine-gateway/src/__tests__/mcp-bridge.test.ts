...
import type { ConversationRuntimeContext, ToolSpec, ToolUseBlock } from "@do-what/protocol";
import { buildAiSdkTools } from "../provider/ai-sdk-tools.js";
...
describe("McpBridge", () => {
  ...
  it("returns the tools handler stub only when hasConversationToolName opts a builtin tool in", async () => {
    ...
    expect(result).toEqual({
      ...
      content: JSON.stringify({ error: "tools.* is not implemented during Phase 0.5" }),
      is_error: true
    });
  });
  ...
  it("buildAiSdkTools includes soul.apply_override and soul.explore_graph", () => {
    const tools = buildAiSdkTools(soulToolDefs);

    expect(Object.keys(tools)).toContain("soul.apply_override");
    expect(Object.keys(tools)).toContain("soul.explore_graph");
  });
  ...
  it("derives soul tool definitions from zod schema into AI SDK Tool shape", async () => {
    const emitCandidateSignal = buildAiSdkTools(soulToolDefs)["soul.emit_candidate_signal"];
    const jsonSchema = await asSchema(emitCandidateSignal.inputSchema).jsonSchema;

    expect(emitCandidateSignal).toMatchObject({
      description: expect.stringContaining("candidate memory signal")
    });
    expect(soulToolDefs.map((toolDef) => toolDef.name)).toContain("soul.emit_candidate_signal");
