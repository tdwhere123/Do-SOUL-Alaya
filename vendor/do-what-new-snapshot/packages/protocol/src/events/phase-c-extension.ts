...
const phaseCExtensionEventTypeValues = [
...
export const PhaseCExtensionEventType = {
...
export const PhaseCExtensionEventTypeSchema = z.enum(phaseCExtensionEventTypeValues);
...
const phaseCExtensionPayloadSchemas = {
  ...
  [PhaseCExtensionEventType.RECALL_EMBEDDING_SUPPLEMENT_DEGRADED]:
  ...
  [PhaseCExtensionEventType.GARDEN_BACKLOG_WARNING]: GardenBacklogWarningPayloadSchema
} as const;

export function createPhaseCExtensionEventObjectSchema<
  T extends keyof typeof phaseCExtensionPayloadSchemas
>(type: T, payloadSchema: (typeof phaseCExtensionPayloadSchemas)[T]) {
  return z.object({ type: z.literal(type), payload: payloadSchema });
}

const ComputeProviderCallStartedEventObjectSchema =
  createPhaseCExtensionEventObjectSchema(
    PhaseCExtensionEventType.COMPUTE_PROVIDER_CALL_STARTED,
    ComputeProviderCallStartedPayloadSchema
  );
...
const ComputeProviderCallFailedEventObjectSchema = createPhaseCExtensionEventObjectSchema(
  PhaseCExtensionEventType.COMPUTE_PROVIDER_CALL_FAILED,
  ComputeProviderCallFailedPayloadSchema
);
...
const GardenBacklogWarningEventObjectSchema = createPhaseCExtensionEventObjectSchema(
...
export const PhaseCExtensionEventUnionSchema = z
...
export type PhaseCExtensionEventPayloadMap = {
  [K in keyof typeof phaseCExtensionPayloadSchemas]: z.infer<
    (typeof phaseCExtensionPayloadSchemas)[K]
  >;
};

export function parsePhaseCExtensionEventPayload<
  T extends keyof typeof phaseCExtensionPayloadSchemas
>(type: T, payload: Record<string, unknown>): PhaseCExtensionEventPayloadMap[T] {
  const schema = phaseCExtensionPayloadSchemas[type];

  if (schema === undefined) {
    throw new Error(`Unknown Phase C extension event type: ${String(type)}`);
  }

  return schema.parse(payload) as PhaseCExtensionEventPayloadMap[T];
}
...
>;
export type PhaseCExtensionEventTypeValue = z.infer<typeof PhaseCExtensionEventTypeSchema>;
export type PhaseCExtensionEvent = z.infer<typeof PhaseCExtensionEventUnionSchema>;
