...
import {
  ...
  PhaseCExtensionEventUnionSchema,
...
describe("Phase C extension protocol schemas", () => {
  ...
  it("registers and parses all C-23 extension event payloads", () => {
    ...
    expect(Object.values(PhaseCExtensionEventType)).toEqual(expectedEventTypes);
    expect(PhaseCExtensionEventTypeSchema.options).toEqual(expectedEventTypes);
    ...
    expect(
      PhaseCExtensionEventUnionSchema.parse({
        type: PhaseCExtensionEventType.GARDEN_BACKLOG_WARNING,
  ...
  it("rejects unknown extension names and malformed extension payloads", () => {
    expect(() => PhaseCExtensionEventTypeSchema.parse("compute.provider.call_unknown")).toThrow();
