...
describe("daemon conversation tool catalog", () => {
  ...
  it("keeps the parsed MCP tool catalog snapshot across refreshes", async () => {
    const originalAllowedServers = process.env.DO_WHAT_ALLOWED_MCP_SERVERS;
    const originalToolCatalog = process.env.DO_WHAT_MCP_TOOL_CATALOG_JSON;
    ...
    try {
      ...
      process.env.DO_WHAT_MCP_TOOL_CATALOG_JSON = "{not valid json";
      ...
      process.env.DO_WHAT_ALLOWED_MCP_SERVERS = originalAllowedServers;
      process.env.DO_WHAT_MCP_TOOL_CATALOG_JSON = originalToolCatalog;
    }
  });
  ...
  it("re-reads the allowed MCP server allow-list on refresh and drops tools from removed servers", async () => {
    ...
    try {
      ...
      process.env.DO_WHAT_ALLOWED_MCP_SERVERS = "github";
  ...
  it("appends and broadcasts extension.tool_discovered on the daemon bootstrap discovery path", async () => {
    ...
    const sseCapture = createSseCapture();
    const connectionId = sseManager.subscribeWorkspace("system", sseCapture.writer);
    sseManager.markReplayComplete(connectionId);
    ...
    try {
      ...
      await waitForCondition(() =>
        sseCapture.text().includes(`event: ${PhaseCEventType.EXTENSION_TOOL_DISCOVERED}`)
      );
      ...
      expect(sseCapture.text()).toContain(`event: ${PhaseCEventType.EXTENSION_TOOL_DISCOVERED}`);
      expect(sseCapture.text()).toContain(`"tool_id":"${discoveredTool.tool_id}"`);
    } finally {
      sseManager.unsubscribeWorkspace(connectionId);
      await sseCapture.writer.closed.catch(() => undefined);
    }
  });
});
