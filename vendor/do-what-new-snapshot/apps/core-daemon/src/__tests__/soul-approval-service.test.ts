...
import { Phase5EventType } from "@do-what/protocol";
...
describe("SoulApprovalService", () => {
  it("throws NOT_FOUND when no pending approval exists for the run", async () => {
    ...
    const sseBroadcaster = { broadcastEntry: vi.fn() };
    ...
    expect(sseBroadcaster.broadcastEntry).not.toHaveBeenCalled();
  });
  ...
  it("preserves the requested message_id and description when resolving an approval", async () => {
    ...
    expect(sseBroadcaster.broadcastEntry).toHaveBeenCalledWith(
