...
export function createDaemonMcpCatalogFromEnv(input: {
  ...
  const isServerAllowed = (serverName: string): boolean =>
    parseAllowedMcpServerNames(env.DO_WHAT_ALLOWED_MCP_SERVERS).includes(serverName);
  const readEnvironment = (): DaemonMcpCatalogEnvironmentSnapshot =>
    Object.freeze({
      allowedServerNames: parseAllowedMcpServerNames(env.DO_WHAT_ALLOWED_MCP_SERVERS),
      rawToolCatalog
    });
...
export function parseDaemonMcpServerRuntimeConfigs(
  ...
  try {
    ...
    warn("failed to parse DO_WHAT_MCP_SERVER_CONFIG_JSON; ignoring MCP runtime config", {
      error
    });
    return Object.freeze({});
  }
...
function parseMcpToolCatalogByServer(
  ...
  try {
    ...
    warn("failed to parse DO_WHAT_MCP_TOOL_CATALOG_JSON; ignoring MCP discovery catalog", {
      error
    });
    return new Map<string, readonly DaemonMcpCatalogToolEntry[]>();
  }
...
function readDaemonMcpCatalogEnvironment(
  ...
  return Object.freeze({
    allowedServerNames: parseAllowedMcpServerNames(env.DO_WHAT_ALLOWED_MCP_SERVERS),
    rawToolCatalog: parseMcpToolCatalogByServer(env.DO_WHAT_MCP_TOOL_CATALOG_JSON, warn)
  });
}
