...
import { parseWorkspaceGitLogLimit } from "@do-what/protocol";
import type { ToolExecutionRecordRepo } from "@do-what/storage";
import {
  createGitDiffService,
  GitCommandError as GitDiffCommandError,
  GitInputError as GitDiffInputError,
  GitTimeoutError as GitDiffTimeoutError,
  type GitDiffService
} from "../git/diff.js";
import {
  createGitLogService,
  GitCommandError as GitLogCommandError,
  GitInputError as GitLogInputError,
  GitTimeoutError as GitLogTimeoutError,
  type GitLogService
} from "../git/log.js";
import {
...
export function registerWorkspaceFileRoutes(
  ...
  const gitDiffService = services.gitDiffService ?? createGitDiffService();
  const gitLogService = services.gitLogService ?? createGitLogService();
  ...
  app.get("/workspaces/:id/files/changed", async (context) => {
    ...
    try {
      ...
      if (error instanceof GitDiffInputError) {
        return context.json(
          {
            success: false,
            error: {
...
function parseOptionalBooleanQuery(value: string | undefined): boolean {
  ...
  throw new GitDiffInputError("include_exec must be true or false");
}
...
function parseGitLogLimit(value: string | undefined): number {
  try {
    ...
    if (error instanceof RangeError) {
      throw new GitDiffInputError(error.message);
    }
...
function mapGitRouteError(
  ...
  if (error instanceof GitDiffInputError || error instanceof GitLogInputError) {
    return context.json(
      {
        success: false,
        error: {
  ...
  if (error instanceof GitDiffTimeoutError || error instanceof GitLogTimeoutError) {
    return context.json(
      {
        success: false,
        error: {
  ...
  if (error instanceof GitDiffCommandError || error instanceof GitLogCommandError) {
    return context.json(
      {
        success: false,
        error: {
