...
import { CoreError, type ConversationService, type RunHotStateService, type RunService } from "@do-what/core";
...
export function registerRunRoutes(app: Hono, services: RunRouteServices): void {
  ...
  app.get("/runs/:id/events", async (context) => {
    ...
    const initialCursor = await services.sseManager.getLatestEventId(runId);

    const stream = new TransformStream<Uint8Array, Uint8Array>();
    const writer = stream.writable.getWriter();
    const connectionId = services.sseManager.subscribe(runId, writer);
    void writer.closed
      .catch(() => {
        // writer.closed rejects on normal client disconnect — no action needed
      })
      .finally(() => {
        services.sseManager.unsubscribe(connectionId);
      });

    void (async () => {
      await services.sseManager.sendConnected(connectionId, runId, initialCursor, isReconnect);

      if (isReconnect) {
        await services.sseManager.replayFrom(runId, lastEventId, connectionId);
      } else {
        await services.sseManager.replayFrom(runId, initialCursor ?? "__do_what_initial_cursor__", connectionId);
      }

      // Mark replay complete so buffered live frames are flushed and subsequent
      // broadcasts are delivered directly, preserving EventLog ordering.
      services.sseManager.markReplayComplete(connectionId);
    })().catch((error: unknown) => {
      logRunRouteWarning(services.warn, "SSE connect init failed", {
        runId,
        connectionId,
