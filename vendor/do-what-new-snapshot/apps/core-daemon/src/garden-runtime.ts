...
type PathGraphSnapshotRepoPort = {
  findLatest(workspaceId: string): Promise<PathGraphSnapshotRecord | null>;
  create(snapshot: PathGraphSnapshotRecord): Promise<PathGraphSnapshotRecord>;
  findHistory(workspaceId: string, limit: number): Promise<readonly PathGraphSnapshotRecord[]>;
...
export function createGardenRuntime(input: {
  ...
  readonly backlogTelemetrySource: GardenBacklogTelemetrySchedulerPort;
  ...
  const schedulerEventLogPort: GardenSchedulerEventLogPort = {
    append: async (entry) => {
      const persisted = await input.eventLogRepo.append({
      ...
      try {
        await input.sseBroadcaster.broadcastEntry(persisted);
      } catch (error) {
        console.warn("[garden] scheduler event broadcast failed", {
  ...
  const gardenScheduler = new GardenScheduler(schedulerEventLogPort, {
  ...
  const orphanDetectionPort: AuditorOrphanDetectionPort | undefined =
    input.orphanDetectionEnabled && orphanRadarRepo !== null
      ? {
          ...
          createOrphanRadarRecord: async (record) => {
            await orphanRadarRepo.create(record);
          }
        }
      : undefined;
  const auditorEventLogPort: AuditorEventLogPort = {
    append: async (entry) => await input.eventLogRepo.append(entry)
  };
  ...
  const backlogTelemetrySource: GardenBacklogTelemetrySchedulerPort = {
