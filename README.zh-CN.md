...
契约。Gate-5F 收口本地 reviewer inbox：assignment、deadline /
overdue 投影、配置后由后端绑定的本地 reviewer identity，以及
MCP / Inspector HTTP / CLI 三条 review 路径的 parity。完整团队
...
*v0.1-closeout 已收口（A2 + Gate-5F）：* append + mutation 这一对现在
已经落进单一 `connection.transaction()` —— `EventPublisher` 的
`appendManyWithMutation` 边界承担这一职责。`(entity_type, entity_id,
revision)` 唯一索引仍然保留作为兜底，但已不再是承载 invariant 的关键
路径。producer service 已全部迁移，Gate-5F 删除了 legacy 的
`publishWithMutation` / `publishManyWithMutation` API。`#BL-022` 和
`#BL-026` 已关闭。
...
*v0.1-closeout 已收口（A3 + Gate-5F）：* 回执现在喂给 Path 轴可塑性：
`PathPlasticityService`（`packages/core/src/path-plasticity-service.ts`）
负责消费 usage proof。Gate-5F 把后台任务挪到 Garden Librarian，并接上
了第四种 plasticity op：`direction_bias` redirection。这个 service
会发 durable 的
`PathRelationReinforced/Weakened/Retired/Redirected`
runtime-governance 事件，`RecallService` 则把可塑性权重和 direction
bias 一起纳入召回评分。
...
> + 2 轮 Codex fix-loop，已合入 `main`。Gate-5F 随后把当前 closeout
> backlog（`#BL-025`..`#BL-036`）拉回 v0.1 范围，在 Phase 6 前收口。
...
Gate-5F closeout 工作都在隔离 worktree 里跑，每张卡都按 review +
fix-loop 纪律收。现在实现卡已经 review-clean，最终 review clean，
full verification 也已通过。
...
- **`#BL-025`** —— 把 `EventPublisherInput` 里那个"必填但被静默
  覆盖"的 `revision` 字段从 source 调用和测试夹具里去掉。
...
- **`#BL-027`** —— 本地 reviewer inbox：assignment、deadline、
  overdue state、配置后由后端绑定的本地 reviewer identity，以及默认单
  reviewer approval。完整团队 quorum 和 escalation 产品流不属于
  这个 v0.1 local-first closeout。
