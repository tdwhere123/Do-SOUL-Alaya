...
*正在收的尾（v0.1 关闭前）：* 目前 daemon 这一侧还没有暴露
pending-proposals 队列，review 记录里也还没带 reviewer 身份——
review 走的是和 propose 同一个 MCP 通道，所以 HITL 是 *agent 编
排* 的，不是 *daemon 编排* 的。最小骨架（`soul.list_pending_proposals`
MCP 工具、`alaya review` CLI 动词、review record 上加
`reviewer_identity`）是 [v0.1 收尾](#接下来的方向) 里的 **A1**
卡。完整的 review-inbox UX（reviewer 分配、deadline、escalation）
留给 v0.2 的 enhancement card。
...
*正在收的尾（v0.1 关闭前）：* append + mutation 这一对目前还没被
包进单一事务；SQLite CAS + `(entity_type, entity_id, revision)`
唯一索引保住了数据一致性，但留下一个很小的竞态窗口。登记为
**#BL-022** —— 新接口 `appendManyWithMutation` 落进单一
`connection.transaction()`，~12 个调用点改成同步 mutate callback。
这是 [v0.1 收尾](#接下来的方向) 里的 **A2** 卡。
...
*正在收的尾（v0.1 关闭前）：* 回执目前喂给 `TrustSummary`，但
**还没喂给 Path 轴的可塑性**（reinforcement / weakening /
redirection / retirement）。schema 字段和 Phase-C 事件 **已经存
在**（`packages/protocol/src/soul/path-relation.ts:113-124`、
`packages/protocol/src/events/phase-c.ts`），缺的只是消费侧的
service。把这条反馈环接上 —— 新建 `PathPlasticityService` + 一
个 Garden tier 处理近期 `UsageProofRecord` —— 是
[v0.1 收尾](#接下来的方向) 里的 **A3** 卡。
...
| 卡 | 关闭的事 | Backlog |
|---|---|---|
| **A1** Daemon HITL 骨架 | `soul.list_pending_proposals` MCP 工具 · `alaya review pending\|accept\|reject` CLI · review record 上加 `reviewer_identity` · Inspector "Pending" 视图 | 新卡 |
| **A2** EventPublisher 原子事务 | `appendManyWithMutation` 落进单一 `connection.transaction()`；~12 个调用点改成同步 mutate；关掉竞态窗口 | `#BL-022` |
