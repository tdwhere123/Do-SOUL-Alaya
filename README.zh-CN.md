...
> 给 CLI 编码 agent（Codex、Claude Code 等）用的 **本地优先**（local-first）记忆核。
> 只有 MCP 与 CLI 接入面，**没有 GUI、没有对话 TUI**。
...
CLI 编码 agent 跨会话会失忆，多个 agent 之间也不共享记忆。Alaya 提供
agent 真正去挂的那一层 **记忆面（memory plane）** —— 持久、被治理、有
证据支撑。Agent 提议、Alaya 决定。所有数据落在你自己控制的 SQLite 文件
里。没有聊天窗口、没有埋点、没有云。

本仓库是 [`do-what-new`](https://github.com/tdwhere123) 项目记忆子系统
的 **port**，上游冻结快照位于 `vendor/do-what-new-snapshot/`。v0.1.0
是第一个把这个 port 端到端接通、并通过 CLI + MCP 暴露出来的版本。
...
Alaya 是 CLI 编码 agent 旁边的 **记忆面**。它持有 agent 的长期记忆：
项目事实、决策、证据、对象间的关系。两条核心理念决定了所有设计：

1. **真相 vs 视图。** 记忆本体（memory ontology）才是持久真相。所有你
   能查到的东西 —— recall 结果、各种投影、Memory Inspector 看到的
   画面 —— 都是 **视图**，不是真相。视图可能错；底下的 capsule 才是
   每个人都得为之负责的部分。
2. **Agent 提议，Alaya 决定。** 接入的 agent（LLM）只发出 *候选*
   （candidate）："这个事实应该被记住"、"这条证据更新了那个 capsule"。
   候选要进入治理流程（Promotion Gate、必要时 HITL），过了才会变成
   持久记忆。**绝不存在 "agent 说啥就自动写入" 的路径。**

具体地，v0.1.0 已经就位的能力：

- 带证据的 **记忆本体**（`MemoryEntry`、`EvidenceCapsule`、
  `SynthesisCapsule`、`ClaimForm`）+ 持久真相的 gating
- **多路径召回** —— 词法、FTS、路径感知、embedding（可选 supplement）
- **被治理的晋升**（governed promotion）—— Promotion Gate + HITL +
  Green 状态状态机
- **会话信任** —— *delivered ≠ used* 这条不变式贯穿端到端
- **Garden 自维护** —— Auditor / Janitor / Librarian + Scheduler，
  全部 fire-and-forget 后台运行
- **Profile / 密钥 / 导入导出 / 可移植备份** 操作集合
- 一个 **MCP server**（`alaya mcp stdio`）暴露 8 个 `soul.*` 工具，外加
  CLI fallback 走同一面
...
- 你从终端驱动 Codex、Claude Code 或类似的 CLI agent。
- 你写代码 / 操作系统时，agent 跨会话的"记忆"问题对你是真痛点。
...
- **数据归属。** 你的记忆在你自己的磁盘上，是一个你随时可以用
  `sqlite3` 打开手动检查的格式。无 SaaS 锁定。
- **离线优先。** Recall、propose、governance 不需要任何网络。可选的
  embedding supplement 可以配，但 v0.1.0 默认关闭。
- **可移植。** `alaya backup` / `export` / `import` 产出可签名的捆绑
  包，可以在不同机器之间搬。
- **可审计。** 治理、配置、import/export、备份、会话信任的每一处变更
  都写进 EventLog —— 它是 append-only 的，是状态回放的真相之源。
...
MCP / CLI → daemon → service → EventLog → DB。Garden 的角色读 EventLog
...
8 个工具，每个都有 schema 上限（zod 派生 JSON Schema；`maxLength`、
`maxItems`、`additionalProperties: false` 在 parse 期与发布的 catalog 里
**双向强制**）。
...
| `soul.explore_graph` | 按边类型 / 方向遍历某个记忆节点的邻居；workspace 由 MCP context 绑定 | 否 |
...
| `alaya install` | install profile 的 plan / apply / rollback（db 路径、embedding、engine 绑定）；写审计 | 是 | 是 |
...
| `alaya tools list` | 列 MCP 工具目录（CLI 走 MCP `tools/list` 的 fallback） | 否 | 否 |
...
每个 mutating 动词都支持 preview-before-write。`attach` / `detach`
...
- 上游子系统在 `do-what-new` 里已经被多个发布周期打磨过了。从零
  re-derive 它的所有决策，会损失数月去逐个发现潜伏 bug。
- Alaya 独有的接入面（CLI、install/attach/detach、doctor、Memory
  Inspector、profile/audit）**确实**是 greenfield —— 但它们包在 port
  好的核心外面，并不替代核心。
...
对于 Alaya 独有的接入面（CLI、install/attach/detach、Memory Inspector
前端）路径不同 —— 那些是 `requires-redesign` 卡。详见不变式 §24。
