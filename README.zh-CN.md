...
[![status](https://img.shields.io/badge/status-v0.1.3-success?style=flat-square)](#接下来的方向)
[![license](https://img.shields.io/badge/license-MIT-green?style=flat-square)](LICENSE)
[![tests](https://img.shields.io/badge/tests-2302%20passing-success?style=flat-square)](#接下来的方向)
...
v0.1.3 把"感知 → 治理 → 沉淀 → 召回 → 回执 → 维护"这条六阶段
记忆环跑在一个 SQLite 文件上闭合。分发走 GitHub Release tarball +
`SHA256SUMS`，由 `scripts/install.sh` 本地校验；npm 是有意不做的。
最近这个 patch（v0.1.3）把 `alaya inspect` 接到了真实 workspace 上，
让 Memory Inspector 在干净环境第一次启动就能正常加载，不再 404。
