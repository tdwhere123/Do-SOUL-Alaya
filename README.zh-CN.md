...
v0.3.11 的 bench surface 通过
[`docs/bench-history`](docs/bench-history/) 发布，稳定归档根如下：
...
`latest-baseline*.json` 只是兼容旧工具的 alias。Release closeout 和 baseline
diff 读取 `latest-passing*`；dashboard 可以读取 `latest-run*` 来展示最新
运行真相。

仓库内跟踪的是 compact diagnostics。完整逐题 diagnostics 写到 release
surface 之外：优先使用 `ALAYA_BENCH_ARTIFACT_ROOT`，否则默认写到 repo-local
`.bench-artifacts/`，compact sidecar 会记录 `full_diagnostics_artifact_path`。

核心 release runs 可复现为：
...
`packages/core/src/relations/path-plasticity/causal-usage-projection.ts`（usage receipt 喂给
Path 轴可塑性）。
...
graph TD
    ...
    subgraph Daemon["apps/core-daemon —— 接线 + 派发"]
        TH["MCP tool handler<br/>(16 个工具：13 soul.* + 3 garden.*)"]
...
**MCP 工具（13 个 `soul.*` + 3 个 `garden.*`）** —— 全部
...
- **边提案治理**：`soul.propose_edge`、
  `soul.list_pending_edge_proposals`、
  `soul.batch_review_edge_proposals`
- **Inline typed resolution**：`soul.resolve`（六种 resolution：
  `confirm` / `reject` / `correct` / `stale` / `defer` /
  `not_relevant`，用于 draft claims 和 staged warnings）
...
> 是当前的信任锚。）
> `.npmignore` 和 `npm pack --dry-run` 只证明 npm artifact 不膨胀；
> GitHub Release source tarball 仍按这里描述的 tarball + `SHA256SUMS`
> 路径校验。
...
走完第 7 步，agent 下次启动就会把 Alaya 当 MCP server 看，16 个
工具（13 个 `soul.*` + 3 个 `garden.*`）在 agent 内部就可以调了。
...
MCP、protocol、EventLog、runtime config 或 SQLite 公共面。v0.3.11 的
live MCP catalog 是 **16 个工具**（13 个 `soul.*` + 3 个 `garden.*`）。
分发仍然走
...
- [`zod`](https://zod.dev) —— 公开 MCP 目录的单一真相源（用其内置 JSON Schema 导出生成工具输入 schema）。
