...
export default function EmbeddingSupplementForm({ onRequiresRestart }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSecretMode(parsed.mode);
      setSecretValue(parsed.value);
      // Surface init/runtime errors next time embedding is exercised. The
      // daemon must restart to pick up new config; until then this re-fetch
      // shows the previous health snapshot, but it primes the form for any
      // stale-but-surfacing failure (e.g., model unknown from a prior save).
