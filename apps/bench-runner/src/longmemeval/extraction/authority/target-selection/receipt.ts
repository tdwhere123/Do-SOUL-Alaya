...
import {
  assertExtractionTargetSelectionReceiptIntegrity as assertReceiptIntegrity,
...
export function createSameRootContinuationTargetSelectionReceipt(input: {
  readonly predecessor: ExtractionTargetSelectionReceipt;
  readonly predecessorAuthorityReceiptDigest: string;
  readonly observation: ExtractionAuthorityObservation;
  readonly now?: Date;
}): ExtractionTargetSelectionReceipt {
  assertReceiptIntegrity(input.predecessor);
  if (!isSha256(input.predecessorAuthorityReceiptDigest)) {
    throw new Error("same-root continuation predecessor authority digest is invalid");
  }
  assertExtractionTargetSelectionInitialWindow(input.predecessor, input.observation);
  assertContinuationFinalIdentity(input.predecessor.final_identity, input.observation);
  return buildTargetSelectionReceipt({
    selectionBasis: {
      ...
      predecessor_authority_receipt_digest: input.predecessorAuthorityReceiptDigest
    },
...
function createTargetSelectionReceipt(input: {
  ...
  return buildTargetSelectionReceipt(input);
}

function buildTargetSelectionReceipt(input: {
  readonly selectionBasis: ExtractionTargetSelectionBasis;
  readonly targetRoot: ExtractionTargetRootBinding;
  readonly observation: ExtractionAuthorityObservation;
  readonly now?: Date;
