...
function buildJudgeUser(
  ...
  if (isAbstention) {
    return buildAbstentionJudgeUser(question, gold, answer);
  }
  if (questionType === "single-session-preference") {
    return buildPreferenceJudgeUser(question, gold, answer);
  }
  return buildStandardJudgeUser(questionType, question, gold, answer);
}

function buildAbstentionJudgeUser(
  question: string,
  gold: string,
  answer: string
): string {
...
function buildPreferenceJudgeUser(
  question: string,
  gold: string,
  answer: string
): string {
...
function buildStandardJudgeUser(
  questionType: string,
  question: string,
  gold: string,
  answer: string
  ...
  return (
    buildStandardJudgeInstruction(questionType) +
    `\n\nQuestion: ${question}\n\nCorrect Answer: ${gold}\n\nModel Response: ${answer}\n\n` +
    "Is the model response correct? Answer yes or no only."
  );
}

function buildStandardJudgeInstruction(questionType: string): string {
  const base =
