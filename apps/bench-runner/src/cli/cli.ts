...
Usage:
  ...
  alaya-bench-runner diagnostic-loop --work-root <dir> --request-manifest <json> [--mode smoke|run|cache-only|report-only] [--from-phase <phase>] [--snapshot <db>] [--snapshot-out <db>] [--query-semantic-factor-cache <json>] [--history-root <path>]
