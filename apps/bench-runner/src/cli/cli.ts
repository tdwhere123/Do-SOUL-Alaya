...
Usage:
  ...
  alaya-bench-runner extraction-fill [--extraction-source-packing reference-eight|singleton] [--variant oracle|s|m] [--limit N] [--offset N] [--concurrency N] [--extraction-initial-concurrency N] [--question-batch-limit N] [--tolerate-provider-task-failures] [--data-dir <path>] [--extraction-cache-root <path>] --extraction-authority <receipt.json> [--extraction-predecessor-authority <receipt.json>] [--extraction-target-selection <receipt.json>] [--pinned-meta-root <path>] [--r3-spend-approval <json>] [--ingestion-mode precomputed_full|lazy_field] [--semantic-artifact-root <path>] [--semantic-max-calls N] [--semantic-max-failures N]
  alaya-bench-runner authorize-extraction [--variant oracle|s|m] [--limit N] [--offset N] [--question-batch-limit N] [--concurrency N] [--data-dir <path>] [--extraction-cache-root <path>] [--pinned-meta-root <path>] --extraction-action probe|fill|sample --extraction-receipt-out <receipt.json> --extraction-output-token-cap N --extraction-output-token-field max_tokens|max_completion_tokens|maxOutputTokens --extraction-input-price-usd-per-million N --extraction-output-price-usd-per-million N --extraction-max-input-tokens N --extraction-disk-floor-bytes N [--extraction-probe-key <sha256>] [--extraction-sample-keys <file>] [--extraction-source-packing reference-eight|singleton] [--extraction-predecessor-authority <receipt.json>] [--extraction-target-selection <receipt.json>] [--repair-invalid-shards]
  ...
  alaya-bench-runner diagnostic-loop --work-root <dir> --request-manifest <json> [--mode smoke|run|cache-only|report-only] [--from-phase <phase>] [--snapshot <db>] [--snapshot-out <db>] [--embedding-cache-overlay <receipt.json>] [--canary-unlock <3q-work-root>] [--history-root <path>]
  ...
  alaya-bench-runner extraction-fill [ordinary fill options] [--extraction-packet-profile <profile.json>] [--extraction-preflight-out <new-report.json> --extraction-preflight-keys <keys.json>] --batch-operation prepare|submit|status|resume|import|cancel --batch-limits <json> [--batch-window <name>] [--batch-request-limit N] [--batch-local-job <id> --batch-remote-job <batches/id>]
