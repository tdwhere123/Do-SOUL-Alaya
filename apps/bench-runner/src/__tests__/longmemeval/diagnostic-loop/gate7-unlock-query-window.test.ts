...
import { assertGate7QueryWindowCompatibility } from
  "../../../bench/diagnostic-loop/gate7-unlock-query-window.js";
...
const WINDOW = [
  GATE7_CANARY_QUERY_TEXTS[GATE7_CANARY_Q1],
  GATE7_CANARY_QUERY_TEXTS[GATE7_CANARY_Q2],
  GATE7_CANARY_QUERY_TEXTS[GATE7_CANARY_Q3]
] as const;
...
describe("Gate 7 query-window compatibility", () => {
  it("admits a 3Q window that is a subset of a larger 100Q cache", async () => {
    ...
    await expect(assertGate7QueryWindowCompatibility(prior, current)).resolves.toBeUndefined();
  });
  ...
  it("fail-closes a missing cache path, digest, or window entry", async () => {
    ...
    await expect(assertGate7QueryWindowCompatibility(withoutPath, current))
      .rejects.toThrow(/missing a bound query cache file/u);

    await expect(assertGate7QueryWindowCompatibility(prior, {
      ...current,
      query_factor_cache: {
    ...
    await expect(assertGate7QueryWindowCompatibility(prior, missingEntry))
  ...
  it("fail-closes a changed overlapping capture or a foreign cache file", async () => {
    ...
    await expect(assertGate7QueryWindowCompatibility(prior, await identityFromFile(changedPath)))
    ...
    await expect(assertGate7QueryWindowCompatibility(prior, foreign))
...
async function identityFromFile(path: string): Promise<ResolvedDiagnosticLoopIdentity> {
  ...
  return {
    ...
    request_identity_digest: "gate7-query-window",
    request: loopRequest(),
...
async function currentRoot(): Promise<string> {
  ...
  const root = await mkdtemp(join(tmpdir(), "gate7-query-window-"));
