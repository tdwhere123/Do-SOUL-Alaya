...
describe("drainEmbeddingWarmupPasses", () => {
  it("reaches all-ready as soon as one backfill pass drains the workspace, well under the maxPasses ceiling", async () => {
    // Single-slot Garden: the first runGardenBackgroundPass that lands the
    // Librarian slot on EMBEDDING_BACKFILL drains the whole workspace in one
    // O(n) handle() call. Earlier passes (slot taken by a competing Librarian
    // kind) make no progress.
    const expected = 50;
