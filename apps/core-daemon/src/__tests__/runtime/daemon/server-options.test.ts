...
import { isRemoteDaemonOptInEnabled, resolveDaemonHostFromEnv, warnIfRemoteDaemonListening } from "../../../runtime/server-options.js";
...
describe("core daemon server listen options", () => {
  ...
  it("allows non-loopback DAEMON_HOST when ALAYA_ALLOW_REMOTE_DAEMON is set", () => {
    const host = resolveDaemonHostFromEnv({
      DAEMON_HOST: "0.0.0.0",
  ...
  it("warns when remote bind is enabled on a non-loopback host", () => {
    ...
    expect(messages.join("\n")).toContain("remote bind enabled");
    expect(messages.join("\n")).toContain("break-glass");
  });
