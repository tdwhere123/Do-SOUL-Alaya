...
describe("mcp memory tool handler", () => {
  ...
  it("rejects attached-agent edge review even when reviewer token binding matches", async () => {
    const deps = createDeps();
    ...
    const rejected = await handler.call({
      toolName: "soul.batch_review_edge_proposals",
      arguments: {
        verdict: "accept",
        filter: { proposal_ids: ["edge-proposal-1"] },
    ...
    expect(deps.edgeProposalService?.batchReview).not.toHaveBeenCalled();
  });

  it("requires reviewer token binding for edge proposal review from a human reviewer surface", async () => {
    const deps = createDeps();
    const handler = createMcpMemoryToolHandler({
      ...deps,
      reviewerIdentityBinding: {
