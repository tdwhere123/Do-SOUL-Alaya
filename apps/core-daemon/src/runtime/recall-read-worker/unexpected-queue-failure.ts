...
import { diagnosticError } from "@do-soul/alaya-protocol";
...
export function reportUnexpectedQueueFailure(
  ...
  try {
    diagnosticError("recall-read-worker unexpected queue failure", payload);
  } catch {
    // Logger failure must not skip process termination.
  } finally {
    process.exit(1);
  }
}
