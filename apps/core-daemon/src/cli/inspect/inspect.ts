...
async function executeInspect(
  ...
  const launchCode = (deps.generateLaunchCode ?? defaultGenerateLaunchCode)();
  let launch: InspectorLaunch | null = null;
  try {
    const launchResult = await launchInspector(ctx, args, deps, checkPortAvailable, token, launchCode);
    if (!launchResult.ok) {
