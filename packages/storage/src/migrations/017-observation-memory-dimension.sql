-- Extend the existing dimension CHECK with low-trust source observations.
-- Preserve rowids, projections, indexes and triggers using the established
-- memory-table rebuild. The runner disables incoming foreign keys outside
-- the transaction and restores enforcement afterward.
...
CREATE TABLE memory_entries_enum_checked (
  ...
  dimension TEXT NOT NULL CHECK (
    dimension IN (
      ...
      'fact', 'hazard', 'glossary', 'episode', 'observation'
    )
  ),
