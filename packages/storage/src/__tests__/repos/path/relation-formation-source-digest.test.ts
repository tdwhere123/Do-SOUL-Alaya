...
} from "../../../repos/path/relation-assertion/source-digest.js";
...
describe("relation formation source digest", () => {
  it("canonicalizes mixed-case keys with the protocol authority", () => {
    ...
    const left = digestRelationFormationEventSource(event({ a: 1, B: 2 }));
    const right = digestRelationFormationEventSource(event({ B: 2, a: 1 }));
    expect(left).toBe(right);
  });
});

function event(
  payloadJson: RelationFormationEventSource["payload_json"]
): RelationFormationEventSource {
  return {
    event_id: "event-1",
  ...
  } satisfies RelationFormationEventSource;
}
