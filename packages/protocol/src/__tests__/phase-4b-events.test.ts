...
describe("Phase 4B protocol schemas", () => {
  ...
  it("parses all phase-4b payloads and adds them to the global event union", async () => {
    const protocol = (await import("../" + "index.js")) as Record<string, unknown>;
    const Phase4BEventType = protocol.Phase4BEventType as Record<string, string>;
    const Phase4BEventTypeSchema = protocol.Phase4BEventTypeSchema as { options: readonly string[]; parse: (value: unknown) => unknown };
    const Phase4BEventUnionSchema = protocol.Phase4BEventUnionSchema as { parse: (value: unknown) => unknown };
    const parsePhase4BEventPayload = protocol.parsePhase4BEventPayload as (
      type: string,
    ...
    expect(parsePhase4BEventPayload(Phase4BEventType.SOUL_GRAPH_EDGE_CREATED, edgeCreatedPayload)).toEqual(
      edgeCreatedPayload
    );
    ...
    expect(
      parsePhase4BEventPayload(Phase4BEventType.SOUL_GRAPH_EXPLORE_COMPLETED, exploreCompletedPayload)
    ).toEqual(exploreCompletedPayload);

    expect(
      parsePhase4BEventPayload(Phase4BEventType.SOUL_GRAPH_EXPLORE_COMPLETED, {
        source_memory_id: "memory-1",
        workspace_id: "workspace-1",
        direction: "both",
        neighbor_count: 2,
    ...
    expect(
      parsePhase4BEventPayload(Phase4BEventType.SOUL_GRAPH_EXPLORE_COMPLETED, topologyExploreCompletedPayload)
    ...
    expect(parsePhase4BEventPayload(Phase4BEventType.SOUL_AUDITOR_POINTER_HEALED, pointerHealedPayload)).toEqual(
    ...
    expect(parsePhase4BEventPayload(Phase4BEventType.SOUL_ORPHAN_RADAR_REPORTED, orphanReportedPayload)).toEqual(
      orphanReportedPayload
    );

    expect(Phase4BEventTypeSchema.options).toEqual([
    ...
    expect(
      Phase4BEventUnionSchema.parse({
        type: Phase4BEventType.SOUL_ORPHAN_RADAR_REPORTED,
    ...
    expect(EventTypeSchema.parse(Phase4BEventType.SOUL_GRAPH_EDGE_CREATED)).toBe(
      Phase4BEventType.SOUL_GRAPH_EDGE_CREATED
    );
    expect(EventTypeSchema.parse(Phase4BEventType.SOUL_AUDITOR_POINTER_HEALED)).toBe(
      Phase4BEventType.SOUL_AUDITOR_POINTER_HEALED
    );
  });
  ...
  it("rejects pointer-healed payloads missing required fields and deprecated field names", async () => {
    ...
    expect(() =>
      parsePhase4BEventPayload(Phase4BEventType.SOUL_AUDITOR_POINTER_HEALED, {
        source_object_id: "claim-1",
        source_object_kind: "claim_form",
        ref_kind: "evidence_ref",
        workspace_id: "workspace-1",
