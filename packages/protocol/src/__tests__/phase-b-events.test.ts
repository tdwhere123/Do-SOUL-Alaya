...
import {
  ...
  PhaseBEventUnionSchema,
  WorkerTrustAssessedPayloadSchema,
  parsePhaseBEventPayload
} from "../events/phase-b.js";
...
describe("Phase B event registry", () => {
  it("registers all Phase B event names and validates each payload type", () => {
    ...
    expect(Object.values(PhaseBEventType)).toEqual(expectedEventTypes);
    expect(PhaseBEventTypeSchema.options).toEqual(expectedEventTypes);
    ...
    expect(parsePhaseBEventPayload(PhaseBEventType.OBLIGATION_CREATED, createdPayload)).toEqual(
      createdPayload
    );
    expect(parsePhaseBEventPayload(PhaseBEventType.OBLIGATION_FULFILLED, fulfilledPayload)).toEqual(
      fulfilledPayload
    );
    expect(parsePhaseBEventPayload(PhaseBEventType.OBLIGATION_EXPIRED, expiredPayload)).toEqual(
      expiredPayload
    );
    expect(
      parsePhaseBEventPayload(PhaseBEventType.OBLIGATION_VIOLATION_BLOCKED, blockedPayload)
    ).toEqual(blockedPayload);
    expect(parsePhaseBEventPayload(PhaseBEventType.DIRTY_STATE_PANIC, panicPayload)).toEqual(
      panicPayload
    );
    expect(parsePhaseBEventPayload(PhaseBEventType.WORKER_TRUST_ASSESSED, trustPayload)).toEqual(
      trustPayload
    );
    expect(
      parsePhaseBEventPayload(PhaseBEventType.NARRATIVE_BUDGET_EXCEEDED, budgetPayload)
    ).toEqual(budgetPayload);
    expect(
      parsePhaseBEventPayload(PhaseBEventType.NARRATIVE_CONSOLIDATION_TRIGGERED, consolidationPayload)
    ).toEqual(consolidationPayload);

    expect(
      PhaseBEventUnionSchema.parse({
        type: PhaseBEventType.OBLIGATION_CREATED,
  ...
  it("rejects unknown names and malformed payloads", () => {
    expect(() => PhaseBEventTypeSchema.parse("obligation.unknown")).toThrow();
    expect(() => EventTypeSchema.parse("dirty.state.panic")).toThrow();

    expect(() =>
      parsePhaseBEventPayload(PhaseBEventType.OBLIGATION_CREATED, {
        obligation_id: "obligation-1",
        kind: "safety_finding",
        state: "pending",
    ...
    expect(() =>
      parsePhaseBEventPayload(PhaseBEventType.DIRTY_STATE_PANIC, {
        dossier_id: "dossier-1",
        worker_run_id: "worker-run-1",
        principal_run_id: "principal-run-1",
        trigger: "manual",
    ...
    expect(() =>
      parsePhaseBEventPayload(PhaseBEventType.WORKER_TRUST_ASSESSED, {
        assessment_id: "assessment-1",
        worker_run_id: "worker-run-1",
        trust_level: "trusted",
        factors: []
