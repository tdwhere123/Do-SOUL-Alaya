...
describe("Phase 5 protocol schemas", () => {
  it("parses file schemas and adds phase-5 events to the global event union", async () => {
    ...
    const Phase5EventType = protocol.Phase5EventType as Record<string, string>;
    const Phase5EventTypeSchema = protocol.Phase5EventTypeSchema as {
      options: readonly string[];
      parse: (value: unknown) => unknown;
    };
    const Phase5EventUnionSchema = protocol.Phase5EventUnionSchema as {
      parse: (value: unknown) => unknown;
    };
    const EventTypeSchema = protocol.EventTypeSchema as {
      parse: (value: unknown) => unknown;
    };
    const parsePhase5EventPayload = protocol.parsePhase5EventPayload as (
      type: string,
    ...
    expect(parsePhase5EventPayload(Phase5EventType.FILE_UPLOADED, uploadedPayload)).toEqual(uploadedPayload);
    const hintPayload = {
    ...
    expect(parsePhase5EventPayload(Phase5EventType.SOUL_HINT_EMITTED, hintPayload)).toEqual(hintPayload);
    expect(parsePhase5EventPayload(Phase5EventType.SOUL_CORRECTION_ISSUED, correctionPayload)).toEqual(
      correctionPayload
    );
    expect(parsePhase5EventPayload(Phase5EventType.SOUL_EXPLANATION_PROVIDED, explanationPayload)).toEqual(
      explanationPayload
    );
    expect(parsePhase5EventPayload(Phase5EventType.SOUL_APPROVAL_REQUESTED, approvalRequestedPayload)).toEqual(
      approvalRequestedPayload
    );
    expect(parsePhase5EventPayload(Phase5EventType.SOUL_APPROVAL_RESOLVED, approvalResolvedPayload)).toEqual(
      approvalResolvedPayload
    );
    expect(Phase5EventTypeSchema.options).toEqual([
    ...
    expect(
      Phase5EventUnionSchema.parse({
        type: Phase5EventType.FILE_UPLOADED,
    ...
    expect(EventTypeSchema.parse(Phase5EventType.FILE_UPLOADED)).toBe(Phase5EventType.FILE_UPLOADED);
    expect(
      Phase5EventUnionSchema.parse({
        type: Phase5EventType.SOUL_APPROVAL_RESOLVED,
    ...
    expect(EventTypeSchema.parse(Phase5EventType.SOUL_APPROVAL_REQUESTED)).toBe(
      Phase5EventType.SOUL_APPROVAL_REQUESTED
    );
  });

  it("rejects invalid file records and phase-5 payloads", async () => {
    const protocol = (await import("../" + "index.js")) as Record<string, unknown>;
    const FileRecordSchema = protocol.FileRecordSchema as {
    ...
    expect(() =>
      parsePhase5EventPayload(Phase5EventType.FILE_UPLOADED, {
        file_id: "11111111-1111-4111-8111-111111111111",
        filename: "bad.bin",
    ...
    expect(() =>
      parsePhase5EventPayload(Phase5EventType.SOUL_APPROVAL_REQUESTED, {
        message_id: "msg_approval_1",
        approval_id: "approval-1",
    ...
    expect(() =>
      parsePhase5EventPayload(Phase5EventType.SOUL_APPROVAL_RESOLVED, {
        message_id: "msg_resolution_1",
        approval_id: "approval-1",
        result: "accepted",
        description: "Apply patch",
