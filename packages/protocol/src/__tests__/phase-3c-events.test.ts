...
import {
  ...
  Phase3CEventUnionSchema,
  parsePhase3CEventPayload
} from "../events/phase-3c.js";

const validTimestamp = "2026-03-25T00:00:00.000Z";

describe("Phase 3C event schemas", () => {
  it("keeps Phase3CEventType enum complete and closed", () => {
    const expected = [
    ...
    expect(Object.values(Phase3CEventType)).toEqual(expected);
    expect(Phase3CEventTypeSchema.options).toEqual(expected);
    expect(() => Phase3CEventTypeSchema.parse("soul.budget.unknown")).toThrow();
  });

  it("parses all phase-3c payloads", () => {
    ...
    expect(parsePhase3CEventPayload(Phase3CEventType.SOUL_BUDGET_DEGRADED, degradedPayload)).toEqual(
      degradedPayload
    );
    ...
    expect(
      parsePhase3CEventPayload(Phase3CEventType.SOUL_BUDGET_BANKRUPTCY_DECLARED, declaredPayload)
    ...
    expect(
      parsePhase3CEventPayload(Phase3CEventType.SOUL_BUDGET_BANKRUPTCY_RESOLVED, resolvedPayload)
    ).toEqual(resolvedPayload);
  });

  it("throws for unknown phase-3c event types", () => {
    expect(() =>
      parsePhase3CEventPayload("soul.budget.unknown" as never, {
        occurred_at: validTimestamp
      })
    ).toThrow();
  });
  ...
  it("rejects negative degraded token counts", () => {
    expect(() =>
      parsePhase3CEventPayload(Phase3CEventType.SOUL_BUDGET_DEGRADED, {
        run_id: "run-1",
        workspace_id: "workspace-1",
        lens_runtime_id: "lens-1",
        steps_applied: [],
  ...
  it("rejects invalid bankruptcy kinds", () => {
    expect(() =>
      parsePhase3CEventPayload(Phase3CEventType.SOUL_BUDGET_BANKRUPTCY_DECLARED, {
        bankruptcy_id: "bankruptcy-1",
  ...
  it("discriminates correctly on type", () => {
    const event = {
      type: Phase3CEventType.SOUL_BUDGET_BANKRUPTCY_RESOLVED,
      payload: {
        bankruptcy_id: "bankruptcy-1",
        proposal_id: "proposal-1",
    ...
    expect(Phase3CEventUnionSchema.parse(event)).toEqual(event);
  });

  it("accepts phase-3c event types in EventType union", () => {
    expect(EventTypeSchema.parse(Phase3CEventType.SOUL_BUDGET_DEGRADED)).toBe(
      Phase3CEventType.SOUL_BUDGET_DEGRADED
    );
    expect(EventTypeSchema.parse(Phase3CEventType.SOUL_BUDGET_BANKRUPTCY_RESOLVED)).toBe(
