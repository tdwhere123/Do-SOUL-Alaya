...
export const PhaseCEventTypeSchema = z.enum(phaseCEventTypeValues);
...
const phaseCPayloadSchemas = {
  [PhaseCEventType.CANONICALIZATION_APPLIED]: CanonicalizationAppliedPayloadSchema,
  [PhaseCEventType.CANONICALIZATION_ALIAS_RESOLVED]: CanonicalizationAliasResolvedPayloadSchema,
  [PhaseCEventType.STANCE_POLICY_EVALUATED]: StancePolicyEvaluatedPayloadSchema,
  [PhaseCEventType.STANCE_RESOLUTION_CHANGED]: StanceResolutionChangedPayloadSchema,
  [PhaseCEventType.COMPUTE_PROVIDER_ROUTED]: ComputeProviderRoutedPayloadSchema,
  [PhaseCEventType.BOOTSTRAPPING_PATHS_PLANTED]: BootstrappingPathsPlantedPayloadSchema,
  [PhaseCEventType.PATH_RELATION_CREATED]: PathRelationCreatedPayloadSchema,
  [PhaseCEventType.PATH_RELATION_REINFORCED]: PathRelationReinforcedPayloadSchema,
  [PhaseCEventType.PATH_RELATION_WEAKENED]: PathRelationWeakenedPayloadSchema,
  [PhaseCEventType.PATH_RELATION_RETIRED]: PathRelationRetiredPayloadSchema,
  [PhaseCEventType.SURFACE_DRIFT_DETECTED]: SurfaceDriftDetectedPayloadSchema,
  [PhaseCEventType.SURFACE_DRIFT_LEASE_ACQUIRED]: SurfaceDriftLeaseAcquiredPayloadSchema,
  [PhaseCEventType.SURFACE_DRIFT_LEASE_RELEASED]: SurfaceDriftLeaseReleasedPayloadSchema,
  [PhaseCEventType.SURFACE_DRIFT_LEASE_RELEASE_FAILED]:
    SurfaceDriftLeaseReleaseFailedPayloadSchema,
  [PhaseCEventType.SURFACE_DRIFT_ALERT]: SurfaceDriftAlertPayloadSchema,
  [PhaseCEventType.PATH_GRAPH_SNAPSHOT_CREATED]: PathGraphSnapshotCreatedPayloadSchema,
  [PhaseCEventType.PATH_CONSOLIDATION_COMPLETED]: PathConsolidationCompletedPayloadSchema,
  [PhaseCEventType.PATH_CONSOLIDATION_FUSED]: PathConsolidationFusedPayloadSchema,
  [PhaseCEventType.OUTPUT_SHAPING_APPLIED]: OutputShapingAppliedPayloadSchema,
  [PhaseCEventType.OUTPUT_COMMAND_COMPRESSED]: OutputCommandCompressedPayloadSchema,
  [PhaseCEventType.MANIFESTATION_BUDGET_EVALUATED]: ManifestationBudgetEvaluatedPayloadSchema,
  [PhaseCEventType.MANIFESTATION_ESCALATION_DECIDED]:
...
function createPhaseCEventObjectSchema<T extends keyof typeof phaseCPayloadSchemas>(
  type: T,
  payloadSchema: (typeof phaseCPayloadSchemas)[T]
) {
  return z.object({
    type: z.literal(type),
...
const CanonicalizationAppliedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.CANONICALIZATION_APPLIED,
  CanonicalizationAppliedPayloadSchema
);
const CanonicalizationAliasResolvedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.CANONICALIZATION_ALIAS_RESOLVED,
  CanonicalizationAliasResolvedPayloadSchema
);
const StancePolicyEvaluatedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.STANCE_POLICY_EVALUATED,
  StancePolicyEvaluatedPayloadSchema
);
const StanceResolutionChangedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.STANCE_RESOLUTION_CHANGED,
  StanceResolutionChangedPayloadSchema
);
const ComputeProviderRoutedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.COMPUTE_PROVIDER_ROUTED,
  ComputeProviderRoutedPayloadSchema
);
const BootstrappingPathsPlantedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.BOOTSTRAPPING_PATHS_PLANTED,
  BootstrappingPathsPlantedPayloadSchema
);
const PathRelationCreatedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_RELATION_CREATED,
  PathRelationCreatedPayloadSchema
);
const PathRelationReinforcedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_RELATION_REINFORCED,
  PathRelationReinforcedPayloadSchema
);
const PathRelationWeakenedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_RELATION_WEAKENED,
  PathRelationWeakenedPayloadSchema
);
const PathRelationRetiredEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_RELATION_RETIRED,
  PathRelationRetiredPayloadSchema
);
const SurfaceDriftDetectedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SURFACE_DRIFT_DETECTED,
  SurfaceDriftDetectedPayloadSchema
);
const SurfaceDriftLeaseAcquiredEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SURFACE_DRIFT_LEASE_ACQUIRED,
  SurfaceDriftLeaseAcquiredPayloadSchema
);
const SurfaceDriftLeaseReleasedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SURFACE_DRIFT_LEASE_RELEASED,
  SurfaceDriftLeaseReleasedPayloadSchema
);
const SurfaceDriftLeaseReleaseFailedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SURFACE_DRIFT_LEASE_RELEASE_FAILED,
  SurfaceDriftLeaseReleaseFailedPayloadSchema
);
const SurfaceDriftAlertEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SURFACE_DRIFT_ALERT,
  SurfaceDriftAlertPayloadSchema
);
const PathGraphSnapshotCreatedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_GRAPH_SNAPSHOT_CREATED,
  PathGraphSnapshotCreatedPayloadSchema
);
const PathConsolidationCompletedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_CONSOLIDATION_COMPLETED,
  PathConsolidationCompletedPayloadSchema
);
const PathConsolidationFusedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.PATH_CONSOLIDATION_FUSED,
  PathConsolidationFusedPayloadSchema
);
const OutputShapingAppliedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.OUTPUT_SHAPING_APPLIED,
  OutputShapingAppliedPayloadSchema
);
const OutputCommandCompressedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.OUTPUT_COMMAND_COMPRESSED,
  OutputCommandCompressedPayloadSchema
);
const ManifestationBudgetEvaluatedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.MANIFESTATION_BUDGET_EVALUATED,
  ManifestationBudgetEvaluatedPayloadSchema
);
const ManifestationEscalationDecidedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.MANIFESTATION_ESCALATION_DECIDED,
  ManifestationEscalationDecidedPayloadSchema
);
const SecurityPassthroughStatusChangedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SECURITY_PASSTHROUGH_STATUS_CHANGED,
  SecurityPassthroughStatusChangedPayloadSchema
);
const SecurityPassthroughInitializationFailedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.SECURITY_PASSTHROUGH_INITIALIZATION_FAILED,
  SecurityPassthroughInitializationFailedPayloadSchema
);
const ExtensionDescriptorRegisteredEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.EXTENSION_DESCRIPTOR_REGISTERED,
  ExtensionDescriptorRegisteredPayloadSchema
);
const ExtensionDescriptorRegistrationRevertedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.EXTENSION_DESCRIPTOR_REGISTRATION_REVERTED,
  ExtensionDescriptorRegistrationRevertedPayloadSchema
);
const ExtensionDescriptorRegistrationCompensationFailedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.EXTENSION_DESCRIPTOR_REGISTRATION_COMPENSATION_FAILED,
  ExtensionDescriptorRegistrationCompensationFailedPayloadSchema
);
const ExtensionToolDiscoveredEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.EXTENSION_TOOL_DISCOVERED,
  ExtensionToolDiscoveredPayloadSchema
);
const ExtensionGovernanceCheckedEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.EXTENSION_GOVERNANCE_CHECKED,
  ExtensionGovernanceCheckedPayloadSchema
);
const ConstitutionalFragmentRegisteredEventObjectSchema = createPhaseCEventObjectSchema(
  PhaseCEventType.CONSTITUTIONAL_FRAGMENT_REGISTERED,
  ConstitutionalFragmentRegisteredPayloadSchema
);
...
export const PhaseCEventUnionSchema = z
...
export type PhaseCEventPayloadMap = {
  [K in keyof typeof phaseCPayloadSchemas]: z.infer<(typeof phaseCPayloadSchemas)[K]>;
};

export function parsePhaseCEventPayload<T extends keyof typeof phaseCPayloadSchemas>(
  type: T,
  payload: Record<string, unknown>
): PhaseCEventPayloadMap[T] {
  const schema = phaseCPayloadSchemas[type];

  if (schema === undefined) {
    throw new Error(`Unknown Phase C event type: ${String(type)}`);
  }

  return schema.parse(payload) as PhaseCEventPayloadMap[T];
}

export type PhaseCEventTypeValue = z.infer<typeof PhaseCEventTypeSchema>;
...
>;
export type PhaseCEvent = z.infer<typeof PhaseCEventUnionSchema>;
