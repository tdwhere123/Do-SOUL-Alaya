...
export async function surfaceBindingServiceBindObject(owner: SurfaceBindingServiceMethodOwner, input: {
    readonly object_id: string;
    readonly surface_id: string;
    readonly is_primary: boolean;
    readonly workspace_id: string;
    ...
    await owner.ensurePrimaryBindingAllowed(parsedInput.object_id, parsedInput.workspace_id, parsedInput.is_primary);
    await owner.ensureCrossCuttingAllowsBinding(parsedInput.object_id, parsedInput.workspace_id, parsedInput.surface_id);

    const driftLeaseId = await owner.acquireDriftLease(
    ...
    try {
      const bindingId = owner.generateObjectId();
      const timestamp = owner.now();
      const binding = owner.buildBinding(parsedInput, timestamp);
      const event = owner.buildBindingCreatedEvent(bindingId, binding);
      const created = await owner.persistBindingWithEvent(binding, bindingId, event);
      operationCompleted = true;
      await owner.classifyBindingDriftSafely({
      ...
      await owner.releaseDriftLeaseSafely({
...
export async function surfaceBindingServiceFindBindingById(owner: SurfaceBindingServiceMethodOwner, bindingId: string): Promise<Readonly<SurfaceBindingRecordView> | null> {
    return await owner.dependencies.surfaceBindingRepo.findByBindingId(parseObjectId(bindingId, "binding_id"));
  }

export async function surfaceBindingServiceTransitionBindingState(owner: SurfaceBindingServiceMethodOwner, bindingId: string, newState: BindingStateType, reason: string, causedBy: string): Promise<Readonly<SurfaceBindingRecordView>> {
    ...
    const existing = await owner.dependencies.surfaceBindingRepo.findByBindingId(parsedBindingId);
    ...
    try {
      ...
      const updated = await owner.requireEventPublisher().appendManyWithMutation([event], () =>
        owner.dependencies.surfaceBindingRepo.updateState(
          existing.binding_id,
          parsedNewState,
          occurredAt
        )
...
export async function surfaceBindingServiceCascadeDetachBySurfaceId(owner: SurfaceBindingServiceMethodOwner, surfaceId: string, workspaceId: string): Promise<void> {
    ...
    const detachTargets = await owner.dependencies.surfaceBindingRepo.findDetachableBySurfaceId(
      parsedSurfaceId,
      parsedWorkspaceId
    );
    ...
    await owner.requireEventPublisher().appendManyWithMutation(events, () =>
      owner.dependencies.surfaceBindingRepo.cascadeDetachBySurfaceId(
        parsedSurfaceId,
        parsedWorkspaceId,
        occurredAt
      )
...
export async function surfaceBindingServiceFindBindingsByObject(owner: SurfaceBindingServiceMethodOwner, objectId: string, workspaceId: string): Promise<readonly Readonly<SurfaceBindingRecordView>[]> {
    return await owner.dependencies.surfaceBindingRepo.findByObjectId(
      parseNonEmptyString(objectId, "object_id"),
      parseNonEmptyString(workspaceId, "workspace_id")
    );
  }

export async function surfaceBindingServiceFindBindingsBySurface(owner: SurfaceBindingServiceMethodOwner, surfaceId: string, workspaceId: string): Promise<readonly Readonly<SurfaceBindingRecordView>[]> {
    return await owner.dependencies.surfaceBindingRepo.findBySurfaceId(
      parseNonEmptyString(surfaceId, "surface_id"),
      parseNonEmptyString(workspaceId, "workspace_id")
    );
  }

export async function surfaceBindingServiceFindBindingsByWorkspace(owner: SurfaceBindingServiceMethodOwner, workspaceId: string): Promise<readonly Readonly<SurfaceBindingRecordView>[]> {
    return await owner.dependencies.surfaceBindingRepo.findByWorkspace(
      parseNonEmptyString(workspaceId, "workspace_id")
    );
  }

export async function surfaceBindingServiceEnsurePrimaryBindingAllowed(owner: SurfaceBindingServiceMethodOwner, objectId: string, workspaceId: string, isPrimary: boolean): Promise<void> {
    if (!isPrimary) {
      return;
    }

    const existingPrimary = await owner.dependencies.surfaceBindingRepo.findPrimaryBinding(objectId, workspaceId);
...
export async function surfaceBindingServiceEnsureCrossCuttingAllowsBinding(owner: SurfaceBindingServiceMethodOwner, objectId: string, workspaceId: string, surfaceId: string): Promise<void> {
    const existingBindings = await owner.dependencies.surfaceBindingRepo.findByObjectId(objectId, workspaceId);
    ...
    const permissionRecord = await owner.dependencies.crossCuttingPermissionLookup.findByObjectId(
      objectId,
      workspaceId
    );
...
export function surfaceBindingServiceBuildBinding(owner: SurfaceBindingServiceMethodOwner, input: ReturnType<typeof parseBindObjectInput>, timestamp: string): Readonly<SurfaceBinding> {
...
export function surfaceBindingServiceBuildBindingCreatedEvent(owner: SurfaceBindingServiceMethodOwner, bindingId: string, binding: Readonly<SurfaceBinding>): SurfaceBindingEventDraft {
    return {
...
export async function surfaceBindingServicePersistBindingWithEvent(owner: SurfaceBindingServiceMethodOwner, binding: Readonly<SurfaceBinding>, bindingId: string, event: SurfaceBindingEventDraft): Promise<Readonly<SurfaceBindingRecordView>> {
    try {
      return await owner.requireEventPublisher().appendManyWithMutation([event], () =>
        owner.dependencies.surfaceBindingRepo.create(binding, bindingId)
      );
    } catch (error) {
...
export async function surfaceBindingServiceAcquireDriftLease(owner: SurfaceBindingServiceMethodOwner, workspaceId: string, operationType: SurfaceDriftOperationType, grantedTo: string): Promise<string | null> {
