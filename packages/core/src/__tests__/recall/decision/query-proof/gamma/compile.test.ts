...
describe("query-compiled Gamma_q", () => {
  it("compiles scalar into one three-stratum atom vocabulary and marks unproved classes unsupported", () => {
    const scalar = compileGamma(scalarQuery(), [candidate("A", { bindings: [binding("alice")] })]);
    expect(scalar.compile_status).toBe("compiled");
    expect(scalar.compile_disposition).toBe("complete");
    ...
    expect(compileGamma(distinctQuery(), [
      candidate("A", { bindings: [binding("alice"), binding("bob")] })
    ]).unsupported_reason).toBe("distinctness_source_unsupported");
    ...
    ]).unsupported_reason).toBe("sequence_slots_source_unsupported");
    expect(compileQueryGamma({
      compilation: compilationFor(scalarQuery()),
      candidates: [candidate("A", { bindings: [binding("alice")] })],
      hypothesis_digest: digestCanonicalQueryV1(scalarQuery())
    }).unsupported_reason).toBe("scalar_simple_source_unproved");
  });
  ...
  it("records all_observable distinct as an unproved class rather than an executable empty Gamma", () => {
  ...
  it("returns explicit unsupported for missing extrema witness and blocked programs", () => {
    expect(compileGamma(argmaxQuery(), [candidate("A")]).unsupported_reason)
      .toBe("extremum_witness_source_unsupported");
    ...
    }).unsupported_reason).toBe("extremum_witness_source_unsupported");
    ...
    expect(partial.compile_status).toBe("compiled");
    expect(partial.compile_disposition).toBe("partial");
    expect(partial.classified_holes.some((row) =>
      row.impacts.includes("blocks_certified_delivery"))).toBe(true);
  });
  ...
  it("uses the receipt-selected hypothesis without inheriting a sibling hypothesis hole", () => {
    ...
    expect(compileQueryGamma({
      compilation,
      candidates: [candidate("A")]
    }).unsupported_reason).toBe("multiple_hypotheses");
  });

  it("does not compile distinct as executable from unproved distinctness", () => {
    expect(compileGamma(distinctQuery(), [
      candidate("A", { bindings: [binding("alice")] }),
      candidate("wrong", { bindings: [binding("alice", "proved_distinct", "y")] })
    ]).compile_status).toBe("unsupported");
  });

  it("does not compile extremum as executable from a witness", () => {
    const query = argmaxQuery();
    const compilation = compilationFor(query);
    const compiled = compileGamma(query, [candidate("A")], {
      compilation,
      extremum_witness: extremumWitness(compilation, "argmax", ["alice"], query)
    });
    expect(compiled.compile_status).toBe("unsupported");
    expect(compiled.unsupported_reason).toBe("extremum_witness_source_unsupported");
    expect(compiled.atoms).toEqual([]);
  });
