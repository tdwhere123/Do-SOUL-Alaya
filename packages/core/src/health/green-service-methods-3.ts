...
export async function greenServiceAssertGrantPreconditions(owner: GreenServiceMethodOwner, params: {
    ...
    if (await owner.isContested(params.memory.object_id, params.workspaceId)) {
      throw new CoreError("CONFLICT", "Contested memory entries cannot enter Green");
    }

    if (await owner.hasOpenCorrection(params.memory.object_id, params.workspaceId)) {
      throw new CoreError("CONFLICT", "Open session overrides block Green grant");
    }

    if (await owner.hasHighRiskGuardHit(params.memory.object_id, params.workspaceId)) {
...
export async function greenServiceGetMemoryOrThrow(owner: GreenServiceMethodOwner, targetObjectId: string): Promise<Readonly<MemoryEntry>> {
    const memory = await owner.dependencies.memoryRepo.findById(targetObjectId);
...
export async function greenServiceIsContested(owner: GreenServiceMethodOwner, targetObjectId: string, workspaceId: string): Promise<boolean> {
    if (owner.dependencies.statusResolver === undefined) {
      owner.warnMissingStatusResolver(workspaceId, targetObjectId);
      return false;
    }

    const governanceRole = await owner.dependencies.statusResolver.getGovernanceRole({
      targetObjectId,
      workspaceId
    });
...
export async function greenServiceHasOpenCorrection(owner: GreenServiceMethodOwner, targetObjectId: string, workspaceId: string): Promise<boolean> {
    return await owner.dependencies.eventLogRepo.hasOpenSessionOverrideCorrection({
      workspaceId,
      targetObjectId,
      nowIso: owner.now()
    });
...
export async function greenServiceHasHighRiskGuardHit(owner: GreenServiceMethodOwner, targetObjectId: string, workspaceId: string): Promise<boolean> {
    return await owner.dependencies.eventLogRepo.hasSecurityHitForTarget({
      workspaceId,
      targetObjectId
    });
  }

export async function greenServiceEvaluatePiercingReason(owner: GreenServiceMethodOwner, params: {
...
export function greenServiceWarnMissingStatusResolver(owner: GreenServiceMethodOwner, workspaceId: string, targetObjectId: string): void {
    if (owner.hasWarnedMissingStatusResolver) {
      return;
    }

    owner.hasWarnedMissingStatusResolver = true;
    owner.warn("[GreenService] statusResolver missing; contested Green checks are disabled.", {
      workspaceId,
      targetObjectId
    });
  }
