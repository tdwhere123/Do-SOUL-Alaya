...
export async function arbitrationServiceResolveSlotConflict(owner: ArbitrationServiceMethodOwner, slotId: string, winnerClaimId: string): Promise<Readonly<Slot>> {
    ...
    await owner.applyWinnerChange(slot, candidates, parsedWinnerClaimId, {
      causedBy: TransitionCausedBy.REVIEW,
      lifecycleReasonPrefix: "manual_resolution",
      eventReasonCode: "manual_resolution"
    });

    const updated = await owner.dependencies.slotRepo.findById(slot.object_id);
...
export async function arbitrationServiceSelectArbitrationDecision(owner: ArbitrationServiceMethodOwner, slot: Readonly<Slot>, candidates: readonly Readonly<ClaimForm>[]): Promise<ArbitrationSelection> {
    ...
    const edges = await owner.loadEdgesForCandidates(candidates);
...
export async function arbitrationServiceApplySelection(owner: ArbitrationServiceMethodOwner, slot: Readonly<Slot>, candidates: readonly Readonly<ClaimForm>[], selection: ArbitrationSelection): Promise<void> {
    if (selection.decision === "contested") {
      await owner.markClaimsContested(candidates, new Set(selection.contestedClaimIds));
      return;
    }

    if (selection.decision === "winner_changed") {
      await owner.applyWinnerChange(slot, candidates, selection.winnerClaimId, {
...
export async function arbitrationServiceMarkClaimsContested(owner: ArbitrationServiceMethodOwner, candidates: readonly Readonly<ClaimForm>[], contestedIds: ReadonlySet<string>): Promise<void> {
...
function collectDecisiveChallengers(
  candidates: readonly Readonly<ClaimForm>[],
  edges: readonly Readonly<ConflictMatrixEdge>[],
...
function buildNoChallengerDecision(
  candidates: readonly Readonly<ClaimForm>[],
  edges: readonly Readonly<ConflictMatrixEdge>[],
  incumbentId: string | null
...
function buildSelectedWinnerDecision(
