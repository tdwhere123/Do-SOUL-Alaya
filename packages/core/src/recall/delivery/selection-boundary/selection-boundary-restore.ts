...
function assertSelectionBoundaryEnvelope(
  ...
  if (schemaVersion === 2) {
    throwSelectionBoundaryFidelityMismatch(
      ...
      "versioned query completeness receipt requires schema_version=5"
    );
  }
  if (schemaVersion !== 5) {
    throwSelectionBoundaryFidelityMismatch(
      `expected schema_version=5, actual ${String(schemaVersion)}`
    );
  }
...
function assertCoverageObjective(value: unknown): void {
  ...
  if (receipt.schema_version !== 1 ||
      receipt.operator_id !== SELECT_GAMMA_BINDING_COVERAGE_OPERATOR_ID ||
