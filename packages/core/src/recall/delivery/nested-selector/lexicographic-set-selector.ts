...
function bestSafeNestedExchange(
  ...
  const activationScenarios = scenarios.filter((scenario) => scenario !== "delivery");
  ...
  const proposals = candidates
    .filter((candidate) => !incumbentKeys.has(candidate.key) &&
      isActivationOpportunity(candidate, activationScenarios, options.packSize))
    .flatMap((challenger) => incumbentHead.flatMap((_, index) => buildSafeExchange({
      challenger,
      evicteeIndex: index,
      incumbentHead,
      incumbentPack,
      scenarios: activationScenarios,
...
function safeSetReplacement(
  ...
  const activationGain = proposedUtility.some((value, index) =>
    value > incumbentUtility[index]! + UTILITY_EPSILON
  );
  const activationLoss = proposedUtility.some((value, index) =>
    value + UTILITY_EPSILON < incumbentUtility[index]!
  );
  if (!demandGain && activationLoss) return false;
  if (!requireStrict) return true;
  return demandGain || activationGain || totalRisk(proposed) < totalRisk(incumbent) ||
    totalCost(proposed) < totalCost(incumbent);
}
...
function totalCost(candidates: readonly NestedSetCandidate[]): number {
...
function objectiveVector(
  ...
  return phase === "head"
    ? [robustUtility, coreCoverage, nominalUtility, supportingCoverage, proposalSupport, -risk, -tokens]
    : [robustUtility, coreCoverage, evidenceCoverage, nominalUtility,
      supportingCoverage, proposalSupport, -risk, -tokens];
}
