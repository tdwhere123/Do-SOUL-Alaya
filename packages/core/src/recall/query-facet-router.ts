...
// Deterministic query->facet hints over FACET_VOCABULARY; populates the facet_overlap stream's querySoughtFacets.
const FACET_HINTS: ReadonlyArray<readonly [string, RegExp]> = [
...
const FACET_HINTS_ACTIVE = FACET_HINTS.filter(([facet]) => FACET_VOCABULARY.includes(facet));
...
export function deriveQuerySoughtFacets(queryProbes: Readonly<RecallQueryProbes>): readonly string[] {
  ...
  return Object.freeze(
    FACET_HINTS_ACTIVE.filter(([, pattern]) => pattern.test(query)).map(([facet]) => facet)
  );
}
