...
const additiveWeightCache = new WeakMap<Readonly<RecallPolicy>, Readonly<ResolvedAdditiveScoringWeights>>();
const fusionWeightCache = new WeakMap<Readonly<RecallPolicy>, ResolvedFusionScoringWeights>();
...
export type EffectiveScorePolicyCache = Readonly<{
  readonly additiveWeights: Required<RecallAdditiveScoringWeights>;
  readonly fusionWeights: Readonly<{
    readonly QUERY_EVIDENCE_BASE_TRANSFER_MAX: number;
    readonly QUERY_EVIDENCE_BASE_WEIGHT_FLOOR: number;
  }>;
  readonly activationWeights: ActivationWeights;
  readonly nowIso: string;
  readonly hasDomainOverrides: boolean;
}>;

export function createEffectiveScorePolicyCache(params: Readonly<{
  readonly policy: Readonly<RecallPolicy>;
  readonly supplementaryData: RecallSupplementaryData;
  readonly nowIso: string;
}>): EffectiveScorePolicyCache {
  const additiveWeights = resolveAdditiveScoringWeights(params.policy);
  return {
    additiveWeights,
    fusionWeights: resolveFusionScoringWeights(params.policy),
    activationWeights: resolveDynamicActivationWeights(
      resolveActivationWeights(),
      params.supplementaryData.graphAndPathColdScore,
      additiveWeights.PATH_PLASTICITY_WEIGHT
    ),
    nowIso: params.nowIso,
    hasDomainOverrides: params.policy.domain_weight_overrides !== undefined
  };
}
...
export function computeEffectiveScoreDetails(params: Readonly<{
  ...
  readonly now: () => string;
  readonly warn: RecallServiceWarnPort;
  readonly policyCache?: EffectiveScorePolicyCache;
...
type EffectiveScoreContext = Readonly<EffectiveScoreParams & {
  ...
  readonly fusionWeights: ResolvedFusionScoringWeights;
...
function resolveEffectiveScoreContext(params: EffectiveScoreParams): EffectiveScoreContext {
  const cache = params.policyCache;
  const additiveWeights = cache?.additiveWeights ?? resolveAdditiveScoringWeights(params.policy);
  ...
  const weights = cache !== undefined && !cache.hasDomainOverrides
    ? cache.activationWeights
    : resolveDynamicActivationWeights(
  ...
  return {
    ...
    additiveWeights,
    fusionWeights: cache?.fusionWeights ?? resolveFusionScoringWeights(params.policy),
    weights,
    canUseMemorySupplement: !isGlobalCandidate && isMemoryCandidate,
    nowIso: cache?.nowIso ?? params.now()
  };
}
...
function computeScoreCalibration(context: EffectiveScoreContext, signals: EffectiveScoreSignals): ScoreCalibration {
  const queryEvidenceTransfer = computeQueryEvidenceBaseTransfer(signals.baseWeight, signals.relevanceFactor, context.fusionWeights);
...
function computeWeightedScoreParts(context: EffectiveScoreContext, signals: EffectiveScoreSignals, calibration: ScoreCalibration): WeightedScoreParts {
  ...
  return { ...parts, score: clamp01(rawScore * context.scoreMultiplier) };
}
...
function resolveAdditiveScoringWeights(
  ...
  const cached = additiveWeightCache.get(policy);
  if (cached !== undefined) {
    return cached;
  }
  ...
  additiveWeightCache.set(policy, resolved);
  return resolved;
}
...
function resolveFusionScoringWeights(
  ...
  const cached = fusionWeightCache.get(policy);
  if (cached !== undefined) {
    return cached;
  }
  ...
  fusionWeightCache.set(policy, resolved);
  return resolved;
}
