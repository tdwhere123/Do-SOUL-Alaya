...
export function scoreQueryEvidenceMatch(
  ...
  const termCoverage = clamp01(hitStats.hitWeight / Math.max(1, termNeedles.length));
...
export function collectQueryTermHits(
  ...
  const content = normalizeEvidenceText(entry.content);
  const metadata = normalizeEvidenceText([...entry.domain_tags, ...entry.evidence_refs].join(" "));
  for (const needle of compileEvidenceNeedles(queryProbes.lexical_terms.slice(0, 32))) {
    if (containsCompiledEvidenceNeedle(content, needle) || containsCompiledEvidenceNeedle(metadata, needle)) {
      hits.add(needle.value);
    }
  }
  return hits;
}
...
function collectEvidenceTermHitStats(
  ...
  for (const needle of needles) {
    const hitInContent = containsCompiledEvidenceNeedle(content, needle);
    const hitInMetadata = !hitInContent && containsCompiledEvidenceNeedle(metadata, needle);
    if (hitInContent) {
      hitWeight += 1;
      contentHits += 1;
...
function countEvidencePhraseHits(
  ...
  return compileEvidenceNeedles(queryProbes.phrases.slice(0, 12))
    .filter((needle) => containsCompiledEvidenceNeedle(content, needle))
    .length;
}
