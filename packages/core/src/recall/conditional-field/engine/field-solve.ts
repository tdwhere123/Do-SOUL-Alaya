import { CONDITIONAL_FIELD_SCHEMA_VERSION, MILLIGRADE_BOTTOM, MILLIGRADE_TOP,
  type CoverageRegion, type FieldValue } from "@do-soul/alaya-protocol";
...
function annotateBounds(state: BindableState, possible: SolveBand, guaranteed: SolveBand): BindMaxMinSuccess {
  ...
  const residualHigh = residualUpper(state.residuals);
  const open = state.residuals.some((region) => region.status === "open" || region.status === "interrupted");
    ...
    snapshot: {
      ...
      get values() {
        materialized ??= bound.snapshot.values.map((value) => value.activation?.kind === "unreachable" ? value : {
          ...value,
          low_milligrades: lower.values.get(productStateNodeId(value.state)) ?? MILLIGRADE_BOTTOM,
          high_milligrades: open ? Math.max(value.milligrades ?? MILLIGRADE_BOTTOM, residualHigh) : value.milligrades
        });
        return materialized;
      }
    }
  ...
  return { size: identities.size, at(index) {
    ...
    return grade === undefined ? { schema_version: 1, state: key, accepting: key.program_state === "accepting", activation: { kind: "unreachable" } }
      : { schema_version: 1, state: key, accepting: key.program_state === "accepting", activation: { kind: "reachable", milligrades: grade },
        milligrades: grade, low_milligrades: binding.guaranteed_values?.get(id) ?? MILLIGRADE_BOTTOM,
        high_milligrades: open ? Math.max(grade, residualHigh) : grade };
  } };
}
...
function residualUpper(residuals: readonly CoverageRegion[]): number {
  ...
  for (const region of residuals) {
    ...
    upper = Math.max(upper, region.conservative_bound_milligrades ?? MILLIGRADE_TOP);
  }
  return upper;
}
