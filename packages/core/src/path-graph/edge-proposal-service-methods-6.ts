...
export async function edgeProposalServiceRecordPathFailureToInbox(owner: EdgeProposalServiceMethodOwner, workspaceId: string, targetObjectId: string): Promise<void> {
    const port = owner.dependencies.healthInboxPort;
    if (port === undefined) {
      return;
    }
    try {
...
export async function edgeProposalServiceRejectProposal(owner: EdgeProposalServiceMethodOwner, proposal: EdgeProposal, reviewerIdentity: string, reviewReason: string | null, reviewedAt: string): Promise<void> {
