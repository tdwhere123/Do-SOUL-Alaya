...
export async function edgeProposalServiceReconcileStuckAccepts(owner: EdgeProposalServiceMethodOwner, input: {
    readonly workspaceId: string;
    readonly limit: number;
  }): Promise<EdgeProposalReconcileSweepResult> {
    const workspaceId = parseObjectId(input.workspaceId);
    const stranded = owner.dependencies.proposalRepo.listAcceptedAwaitingPath(workspaceId, input.limit);
...
export function edgeProposalServiceDefaultExpiresAt(owner: EdgeProposalServiceMethodOwner, createdAt: string): string {
