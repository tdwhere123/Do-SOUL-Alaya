...
export async function edgeProposalServiceSweepExpired(owner: EdgeProposalServiceMethodOwner, input: {
    readonly workspaceId: string;
    readonly limit: number;
  }): Promise<EdgeProposalExpirySweepResult> {
    const workspaceId = parseObjectId(input.workspaceId);
    const nowIso = owner.now();
    const candidates = owner.dependencies.proposalRepo.listExpiredPending(workspaceId, nowIso, input.limit);
    let expired = 0;
    let skipped = 0;
    for (const proposal of candidates) {
      const reviewedAt = owner.now();
      try {
        await owner.dependencies.eventPublisher.appendManyWithMutation(
          ...
          () => {
            owner.dependencies.proposalRepo.updateReview({
...
export async function edgeProposalServiceHandleMintFailure(owner: EdgeProposalServiceMethodOwner, proposal: EdgeProposal, acceptedStatus: typeof EdgeProposalStatus.ACCEPTED | typeof EdgeProposalStatus.AUTO_ACCEPTED, reviewerIdentity: string, failureKind: "submit_returned_false" | "submit_threw", mintOutcome: "failed" | "rejected", cause: unknown = null): Promise<void> {
    const reviewedAt = owner.now();
    ...
    await owner.dependencies.eventPublisher.appendManyWithMutation(
      ...
      () => {
        owner.dependencies.proposalRepo.reconcileAfterMintFailure({
          proposalId: proposal.proposal_id,
    ...
    await owner.recordPathFailureToInbox(proposal.workspace_id, proposal.source_memory_id);
  }
