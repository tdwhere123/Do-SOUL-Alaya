...
import {
  ...
  type MemoryGraphEdgeTypeValue} from "@do-soul/alaya-protocol";
...
export function edgeProposalServiceRequireExplicitProposalIdsSelected(owner: EdgeProposalServiceMethodOwner, filter: EdgeProposalFilter, proposals: readonly EdgeProposal[]): void {
...
export async function edgeProposalServiceAcceptProposal(owner: EdgeProposalServiceMethodOwner, proposal: EdgeProposal, reviewerIdentity: string, reviewReason: string | null, reviewedAt: string, acceptedStatus: typeof EdgeProposalStatus.ACCEPTED | typeof EdgeProposalStatus.AUTO_ACCEPTED): Promise<EdgeProposal> {
    await owner.requireMemoryInWorkspace(proposal.source_memory_id, "Source", proposal.workspace_id);
    await owner.requireMemoryInWorkspace(proposal.target_memory_id, "Target", proposal.workspace_id);
    ...
    await owner.dependencies.eventPublisher.appendManyWithMutation(
      ...
      () => {
        reviewed = owner.dependencies.proposalRepo.updateReview({
    ...
    // loud at call time.
    const outcome = await owner.mintAcceptedPath(proposal, reviewerIdentity, acceptedStatus);
...
export async function edgeProposalServiceMintAcceptedPath(owner: EdgeProposalServiceMethodOwner, proposal: EdgeProposal, reviewerIdentity: string, acceptedStatus: typeof EdgeProposalStatus.ACCEPTED | typeof EdgeProposalStatus.AUTO_ACCEPTED): Promise<PathMintOutcome> {
    ...
    try {
      outcome = await owner.dependencies.pathCandidatePort.submitCandidate({
      ...
      await owner.handleMintFailure(proposal, acceptedStatus, reviewerIdentity, "submit_threw", "failed", mintError);
      return "failed";
    }
    if (outcome === "failed" || outcome === "rejected") {
      await owner.handleMintFailure(proposal, acceptedStatus, reviewerIdentity, "submit_returned_false", outcome);
    }
    return outcome;
  }
