...
export async function edgeProposalServiceBatchReview(owner: EdgeProposalServiceMethodOwner, input: {
    readonly workspaceId: string;
    ...
    const proposals = owner.dependencies.proposalRepo.listPending(workspaceId, input.filter);
    owner.requireExplicitProposalIdsSelected(input.filter, proposals);
    ...
    for (const proposal of proposals) {
      if (input.verdict === "accept") {
        ...
        await owner.rejectProposal(proposal, input.reviewerIdentity, input.reason, reviewedAt);
        rejectedCount += 1;
      }
...
export async function edgeProposalServiceProposeExplicitEdge(owner: EdgeProposalServiceMethodOwner, input: {
    readonly sourceMemoryId: string;
    readonly targetMemoryId: string;
    readonly edgeType: MemoryGraphEdgeTypeValue;
    readonly confidence: number;
    ...
    const proposal = await owner.proposeEdge({
...
export async function edgeProposalServiceRequireMemoryInWorkspace(owner: EdgeProposalServiceMethodOwner, memoryId: string, label: string, workspaceId: string): Promise<void> {
    const memory = await owner.dependencies.memoryRepo.findById(memoryId);
    if (memory === null) {
