...
it("isolates nested generation schema mutations between callers", () => {
  ...
  const changed = officialApiExtractionResponseSchema(prompt) as {
    properties: { interpretations: { items: { properties: Record<string, unknown> } } }
  };
...
it("describes the live interpretation envelope without kind or confidence", () => {
  const schema = officialApiExtractionResponseSchema(JSON.stringify(request)) as {
    properties: { interpretations: { items: { required: string[]; properties: Record<string, unknown> } } }
  };
  const entry = schema.properties.interpretations.items;
  expect(entry.required).toEqual(expect.arrayContaining(["assertion_id", "relations"]));
  expect(entry.properties).not.toHaveProperty("object_kind");
  expect(entry.properties).not.toHaveProperty("confidence");
  expect(entry.properties).not.toHaveProperty("temporal_projection");
});
